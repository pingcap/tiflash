...
4. The cost-efficiency is not optimal. For example, TiFlash instances may need to be kept running even during periods of low query demand.
...
2. There is no suitable shared storage for storing all data
    ...
    * AWS's EBS does have IOPS types that can be accessed by multiple nodes, but it has many limitations, does not meet the needs of distributed databases for shared storage, and is expensive.
...
2. IO latency sensitive data is directly written to Write Node's local disk and uploaded to S3 peroidically. I.e. it uses local disk as buffer
