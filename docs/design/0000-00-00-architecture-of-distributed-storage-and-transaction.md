...
It's also convenient for PD to maintains this kind of storage system by existing way as long as it works as `raft store`.
...
No matter for learner/follower or even leader, `Read Index` is a effective choice to check latest `Lease`.
...
However, the region snaoshot data, which in form of TiKV `SST` file, is really hard to used by other storage system directly.
...
In concept of `Snapshot Isolation`, all deletion operations proposed by transaction process is logical, which means `Put` a key-value with write_type `D`(delete).
