...
### Bootstrap an Isolated Environment (Recommended)

1. execute `./bootstrap.sh`, and all dependencies should be ready, with `curl` and `grpc` in `deps` dir.
2. `cd build && make tiflash`
3. get TiFlash binary under `build/dbms/src/Server/tiflash`.


### Set Up Your Global Environment
