...
function wait_next_gen_columnar_env() {
  local timeout='200'
  local failed='true'

  echo "=> wait for columnar env available"
  ...
  for (( i = 0; i < "${timeout}"; i++ )); do
    ...
    if [ $((${i} % 10)) = 0 ] && [ ${i} -ge 10 ]; then
      echo "   #${i} waiting for columnar env available"
    fi
  ...
  if [ "${failed}" == 'true' ]; then
    echo "   can not set up columnar env" >&2
    exit 1
  else
    echo "   available"
  fi
...
function prepare_next_gen_columnar_data_dirs() {
  mkdir -p \
    ./data/minio ./log/minio \
    ./data/pd0 ./log/pd0 \
    ./data/tikv0 ./log/tikv0 \
...
export -f wait_next_gen_columnar_env
export -f set_branch
export -f clean_data_log
export -f prepare_next_gen_data_dirs
export -f prepare_next_gen_columnar_data_dirs
export -f check_env
