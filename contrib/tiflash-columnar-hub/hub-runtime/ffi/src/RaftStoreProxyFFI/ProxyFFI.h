...
struct SSTReaderInterfaces
{
    ...
    void (*fn_seek)(SSTReaderPtr, ColumnFamilyType, EngineIteratorSeekType, BaseBuffView);
...
struct CloudStorageEngineInterfaces
{
    ...
    RustStrWithViewVec (*fn_get_region_bucket_keys)(uint64_t, uint64_t, RaftStoreProxyPtr);
    void (*fn_clear_shared_snap_access_by_start_ts)(uint64_t, RaftStoreProxyPtr);
    ColumnarReaderPtr (*fn_get_columnar_reader)(
        uint64_t,
        uint64_t,
        uint64_t,
        BaseBuffView,
...
struct ColumnarLateMaterializationInterfaces
{
    uint32_t version;
    uint32_t size;
    uint64_t (*fn_read_early_block)(ColumnarReaderPtr, uint64_t, BaseBuffView, uint64_t *, int64_t *);
    RustStrWithView (*fn_read_early_column)(ColumnarReaderPtr, uint64_t, int64_t);
    uint64_t (*fn_materialize_selected)(ColumnarReaderPtr, uint64_t, uint8_t, BaseBuffView);
    RustStrWithView (*fn_read_late_column)(ColumnarReaderPtr, uint64_t, int64_t);
    uint8_t (*fn_finish_materialized_block)(ColumnarReaderPtr, uint64_t);
    uint8_t (*fn_discard_late_materialization_batch)(ColumnarReaderPtr, uint64_t);
    uint8_t (*fn_is_late_materialization_supported)(ColumnarReaderPtr, BaseBuffView);
};

#if defined(__GNUC__) || defined(__clang__)
extern "C" const ColumnarLateMaterializationInterfaces * tiflash_columnar_get_late_materialization_interfaces()
    __attribute__((weak));
#endif
...
enum class ConfigJsonType : uint64_t
{
...
struct RaftStoreProxyFFIHelper
{
    ...
    RaftstoreVer (*fn_get_cluster_raftstore_version)(RaftStoreProxyPtr, uint8_t refresh_strategy, int64_t timeout_ms);
...
struct PageStorageInterfaces
{
    ...
    RawCppPtrCarr (*fn_handle_scan_page)(const EngineStoreServerWrap *, BaseBuffView, BaseBuffView);
    CppStrWithView (*fn_handle_get_lower_bound)(const EngineStoreServerWrap *, BaseBuffView);
...
struct EngineStoreServerHelper
{
    ...
    EngineStoreApplyRes (
        *fn_handle_admin_raft_cmd)(const EngineStoreServerWrap *, BaseBuffView, BaseBuffView, RaftCmdHeader);
    uint8_t (*fn_need_flush_data)(EngineStoreServerWrap *, uint64_t);
    uint8_t (*fn_try_flush_data)(
    ...
    RawCppPtr (
        *fn_pre_handle_snapshot)(EngineStoreServerWrap *, BaseBuffView, uint64_t, SSTViewVec, uint64_t, uint64_t);
    void (*fn_apply_pre_handled_snapshot)(EngineStoreServerWrap *, RawVoidPtr, RawCppPtrType);
    void (*fn_abort_pre_handle_snapshot)(EngineStoreServerWrap *, uint64_t, uint64_t);
    void (*fn_release_pre_handled_snapshot)(EngineStoreServerWrap *, RawVoidPtr, RawCppPtrType);
    uint8_t (*fn_apply_fap_snapshot)(EngineStoreServerWrap *, uint64_t, uint64_t, uint8_t, uint64_t, uint64_t);
    HttpRequestRes (
        *fn_handle_http_request)(EngineStoreServerWrap *, BaseBuffView path, BaseBuffView query, BaseBuffView body);
    ...
    void (*fn_set_pb_msg_by_bytes)(MsgPBType type, RawVoidPtr ptr, BaseBuffView buff);
    void (*fn_handle_safe_ts_update)(
        EngineStoreServerWrap *,
        uint64_t region_id,
        uint64_t self_safe_ts,
        uint64_t leader_safe_ts);
    FastAddPeerRes (*fn_fast_add_peer)(EngineStoreServerWrap *, uint64_t region_id, uint64_t new_peer_id);
    CppStrWithView (*fn_get_lock_by_key)(const EngineStoreServerWrap *, uint64_t, BaseBuffView);
    FapSnapshotState (*fn_query_fap_snapshot_state)(
        EngineStoreServerWrap *,
        uint64_t region_id,
        uint64_t new_peer_id,
        uint64_t,
        uint64_t);
    void (*fn_clear_fap_snapshot)(EngineStoreServerWrap *, uint64_t region_id, uint64_t state);
    bool (*fn_kvstore_region_exists)(EngineStoreServerWrap *, uint64_t region_id);
    void (*fn_report_thread_allocate_info)(
        EngineStoreServerWrap *,
        uint64_t thread_id,
        BaseBuffView name,
        ReportThreadAllocateInfoType type,
        uint64_t value);
    void (*fn_report_thread_allocate_batch)(
        EngineStoreServerWrap *,
        uint64_t thread_id,
        BaseBuffView name,
...
uint32_t ffi_get_server_info_from_proxy(intptr_t, DB::BaseBuffView, DB::RawVoidPtr);
#ifdef __cplusplus
}
#endif
