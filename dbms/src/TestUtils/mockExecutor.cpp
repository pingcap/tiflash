...
DAGRequestBuilder & DAGRequestBuilder::exchangeReceiver(const MockColumnInfos & columns, uint64_t fine_grained_shuffle_stream_count)
{
    return buildExchangeReceiver(columns, fine_grained_shuffle_stream_count);
}

DAGRequestBuilder & DAGRequestBuilder::exchangeReceiver(const MockColumnInfoList & columns, uint64_t fine_grained_shuffle_stream_count)
{
    return buildExchangeReceiver(columns, fine_grained_shuffle_stream_count);
}

DAGRequestBuilder & DAGRequestBuilder::buildExchangeReceiver(const MockColumnInfos & columns, uint64_t fine_grained_shuffle_stream_count)
{
    DAGSchema schema;
    for (const auto & column : columns)
    {
