    ...
    bool enable_pipeline)
{
    ...
    ExecutionSummaryCollector summary_collector(dag_context, enable_pipeline, true);
    auto summaries = summary_collector.genExecutionSummaryResponse().execution_summaries();
    ASSERT_EQ(summaries.size(), expect.size()) << "\n"
                                               << testInfoMsg(request, !enable_pipeline, enable_pipeline, concurrency, DEFAULT_BLOCK_SIZE);
