...
template <typename Collator>
int Pattern<Collator>::try_match_ascii_ci(const char * s, size_t length) const
{
    size_t p_idx = 0;
        ...
        if (p_idx_after_any > 0)
        {
            ...
            { // Fast forward to the first match position
                while (str_idx < length)
                {
                    ...
                    if (weight_ascii_ci[s[str_idx]] != weight_ascii_ci[ascii_ci_pattern[p_idx_after_any]])
                    {
                        str_idx = ++backtrack_idx;
                    }
                    else
