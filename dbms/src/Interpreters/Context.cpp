...
void Context::createTMTContext(const TiFlashRaftConfig & raft_config, pingcap::ClusterConfig && cluster_config, const ServerInfo & server_info)
{
    auto lock = getLock();
    if (shared->tmt_context)
        throw Exception("TMTContext has already existed", ErrorCodes::LOGICAL_ERROR);
    shared->tmt_context = std::make_shared<TMTContext>(*this, raft_config, cluster_config, server_info);
}
