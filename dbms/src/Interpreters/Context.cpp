...
BackgroundProcessingPool & Context::initializeBackgroundPool(UInt16 pool_size)
{
    ...
    if (!shared->background_pool)
        shared->background_pool = std::make_shared<BackgroundProcessingPool>(pool_size, "bg-", getJointThreadInfoJeallocMap(lock));
...
BackgroundProcessingPool & Context::initializeBlockableBackgroundPool(UInt16 pool_size)
{
    ...
    if (!shared->blockable_background_pool)
        shared->blockable_background_pool = std::make_shared<BackgroundProcessingPool>(pool_size, "bg-block-", getJointThreadInfoJeallocMap(lock));
...
BackgroundProcessingPool & Context::getPSBackgroundPool()
{
    ...
    if (!shared->ps_compact_background_pool)
        shared->ps_compact_background_pool
            = std::make_shared<BackgroundProcessingPool>(settings.background_pool_size, "bg-page-", getJointThreadInfoJeallocMap(lock));
