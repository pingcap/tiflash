...
#define APPLY_FOR_SETTINGS(M) \
    ...
    M(SettingUInt64, dt_segment_bg_gc_check_interval, 600, "Background gc thread check interval, the unit is second.")\
    M(SettingInt64, dt_segment_bg_gc_max_segments_to_check_every_round, 30, "Max segments to check in every gc round, value less than or equal to 0 means gc no segments.")\
    M(SettingFloat, dt_segment_bg_gc_ratio_threhold_to_trigger_gc, 1.2, "Trigger segment's gc when the ratio of invalid version exceed this threhold. Values smaller than or equal to 1.0 means gc all segments")\
