...
#define APPLY_FOR_SETTINGS(M)                                                                                                                                                                                                           \
    ...
    M(SettingDouble, pipeline_keyspace_cpu_limit_ratio, 0.0, "Maximum pipeline CPU-time share for one keyspace. CPU and IO tasks share the limit. 0 disables CPU-time limiting. Only has meaning at server startup.")                \
    M(SettingDouble, pipeline_keyspace_pool_limit_ratio, 0.0, "Maximum pipeline active-task share for one keyspace. CPU and IO tasks share the limit. 0 disables concurrent-task limiting. Only has meaning at server startup.")       \
    ...
    M(SettingBool, enable_columnar_l2_late_materialization, false, "Enable L2-only late materialization for cloud columnar reads")                                                                                                    \
    M(SettingFloat, columnar_l2_late_materialization_min_selection_skip_ratio, 0.5, "Minimum exact selection skip ratio required for L2 late materialization")                                                                  \
    M(SettingFloat, columnar_l2_late_materialization_min_late_to_early_ratio, 10.0, "Minimum late-column to (early-column minus handle/version) ratio for L2 late materialization")                                                \
