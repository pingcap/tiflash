...
#define APPLY_FOR_SETTINGS(M)                                                                                                              \
    ...
    M(SettingUInt64, mpp_task_waiting_timeout, DEFAULT_MPP_TASK_WAITING_TIMEOUT,                                                           \
    ...
    M(SettingUInt64, batch_commands_threads, 0,                                                                                            \
        "Number of threads to use for handling batch commands concurrently. 0 means - same as 'max_threads'.")                             \
    ...
    M(SettingUInt64, batch_cop_pool_size, 0,                                                                                               \
    ...
    M(SettingUInt64, background_pool_size, DBMS_DEFAULT_BACKGROUND_POOL_SIZE,                                                              \
        "Number of threads performing background work for tables (for example, merging in merge tree). Only has meaning at server "        \
        "startup.")                                                                                                                        \
                                                                                                                                           \
    ...
    M(SettingUInt64, max_parallel_replicas, 1,                                                                                             \
        "The maximum number of replicas of each shard used when the query is executed. For consistency (to get different parts of the "    \
    ...
    M(SettingBool, distributed_group_by_no_merge, false,                                                                                   \
        "Do not merge aggregation states from different servers for distributed query processing - in case it is for certain that there "  \
    ...
    M(SettingBool, merge_tree_uniform_read_distribution, true,                                                                             \
        "Distribute read from MergeTree over threads evenly, ensuring stable average execution time of each thread within one read "       \
        "operation.")                                                                                                                      \
                                                                                                                                           \
    ...
    M(SettingBool, force_primary_key, 0, "Throw an exception if there is primary key in a table, and it is not used.")                     \
                                                                                                                                           \
    M(SettingUInt64, mark_cache_min_lifetime, 0,                                                                                           \
    ...
    M(SettingUInt64, read_backoff_max_throughput, 1048576,                                                                                 \
        "Settings to reduce the number of threads in case of slow reads. Count events when the read bandwidth is less than that many "     \
        "bytes per second.")                                                                                                               \
    M(SettingMilliseconds, read_backoff_min_interval_between_events_ms, 1000,                                                              \
        "Settings to reduce the number of threads in case of slow reads. Do not pay attention to the event, if the previous one has "      \
    ...
    M(SettingBool, send_progress_in_http_headers, false,                                                                                   \
        "Send progress notifications using X-ClickHouse-Progress headers. Some clients do not support high amount of HTTP headers "        \
    ...
    M(SettingBool, fsync_metadata, 1,                                                                                                      \
        "Do fsync after changing metadata for tables and databases (.sql files). Could be disabled in case of poor latency on server "     \
        "with high load of DDL queries and high load of disk subsystem.")                                                                  \
                                                                                                                                           \
    M(SettingUInt64, input_format_allow_errors_num, 0,                                                                                     \
        "Maximum absolute amount of errors while reading text formats (like CSV, TSV). In case of error, if both absolute and relative "   \
    ...
    M(SettingFloat, input_format_allow_errors_ratio, 0,                                                                                    \
        "Maximum relative amount of errors while reading text formats (like CSV, TSV). In case of error, if both absolute and relative "   \
        "values are non-zero, and at least absolute or relative amount of errors is lower than corresponding value, will skip until next " \
        "line and continue.")                                                                                                              \
                                                                                                                                           \
    ...
    M(SettingUInt64, insert_distributed_timeout, 0,                                                                                        \
        ...
        "timeout.")                                                                                                                        \
    M(SettingInt64, distributed_ddl_task_timeout, 180,                                                                                     \
...
<<<<<<< HEAD
    ...
    M(SettingUInt64, max_rows_to_read, 0,                                                                                                  \
        "Limit on read rows from the most 'deep' sources. That is, only in the deepest subquery. When reading from a remote server, it "   \
        "is only checked on a remote server.")                                                                                             \
    M(SettingUInt64, max_bytes_to_read, 0,                                                                                                 \
        "Limit on read bytes (after decompression) from the most 'deep' sources. That is, only in the deepest subquery. When reading "     \
    ...
    M(SettingUInt64, max_expanded_ast_elements, 500000,                                                                                    \
    ...
    M(SettingUInt64, mutable_deduper, 5,                                                                                                   \
        "The deduper used by MutableMergeTree storage. By default 5. 0: OriginStreams, 1: OriginUnity, 2: ReplacingUnity, 3: "             \
    ...
    M(SettingUInt64, dt_segment_delta_cache_limit_size, 4194304,                                                                           \
    ...
    M(SettingInt64, dt_bg_gc_max_segments_to_check_every_round, 100,                                                                       \
        "Max segments to check in every gc round, value less than or equal to 0 means gc no segments.")                                    \
    M(SettingFloat, dt_bg_gc_ratio_threhold_to_trigger_gc, 1.2,                                                                            \
        "Trigger segment's gc when the ratio of invalid version exceed this threhold. Values smaller than or equal to 1.0 means gc all "   \
        "segments")                                                                                                                        \
    M(SettingFloat, dt_bg_gc_delta_delete_ratio_to_trigger_gc, 0.3,                                                                        \
    ...
    M(SettingFloat, dt_page_num_max_gc_valid_rate, 1.0,                                                                                    \
        "Max valid rate of deciding a page file can be compact when exising legacy files are more over than "                              \
    ...
    M(SettingUInt64, max_network_bandwidth_for_user, 0,                                                                                    \
        ...
        "unlimited.")                                                                                                                      \
    M(SettingUInt64, max_network_bandwidth_for_all_users, 0,                                                                               \
...
>>>>>>> 677ad751e3 (increase bg gc check interval (#5056))
