...
public:
    void SetUp(const ::benchmark::State &) override
    {
        try
        {
            ...
            LOG_INFO(log, "gjt debug setup beg");

            test_blocks = generateData(20000, 4096);
            ...
            analyzer.buildAggGroupBy(agg_tipb.group_by(), before_agg_actions, aggregate_desc, aggregated_columns,
                    aggregation_keys, agg_key_set, /*collation sensitive*/true, collators);
...
try
{
    for (auto _ : state)
    {
        ...
        LOG_INFO(log, "gjt debug build_side_watch build: {}", build_side_watch.getAggBuild());
        ...
        res_block.push_back(merging_buckets->getData(0));
    }
}
CATCH
