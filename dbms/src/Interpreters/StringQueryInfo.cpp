...
namespace DB
{
    ...
    StringQueryInfo::StringQueryInfo(const char *begin_, const char *end_, bool internal_)
    : begin(begin_), end(end_), internal(internal_){}
    std::tuple<std::string, ASTPtr> StringQueryInfo::parse(size_t max_query_size) {
    ...
    String StringQueryInfo::get_query_ignore_error(size_t max_query_size) {
        return String(begin, begin + std::min(end - begin, static_cast<ptrdiff_t>(max_query_size)));
    }

    std::unique_ptr<IInterpreter> StringQueryInfo::getInterpreter(Context & context, QueryProcessingStage::Enum stage) {
