...
size_t JoinPartition::getRowCount()
{
    ...
    ret += getRowCountImpl(maps_all_full_with_row_flag, join_type);
    return ret;
}
...
size_t JoinPartition::getHashMapAndPoolByteCount()
{
    ...
    ret += getByteCountImpl(maps_all_full_with_row_flag, join_type);
    ret += pool->size();
    return ret;
}
...
void JoinPartition::initMap()
{
    ...
    if (isReverseJoin(kind))
    {
        if (has_other_condition)
            initImpl(maps_all_full_with_row_flag, join_type);
        else
...
void JoinPartition::releasePartitionPoolAndHashMap(std::unique_lock<std::mutex> &)
{
    ...
    released_bytes += clearMaps(maps_all_full_with_row_flag, join_type);
    subMemoryUsage(released_bytes);
}
