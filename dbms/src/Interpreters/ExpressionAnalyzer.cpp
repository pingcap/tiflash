...
{
    // Is this code not used by TiFlash?
    if (only_types)
        actions->add(ExpressionAction::ordinaryJoin(0, nullptr, columns_added_by_join));
    else
        for (auto & subquery_for_set : subqueries_for_sets)
            if (subquery_for_set.second.join)
                actions->add(ExpressionAction::ordinaryJoin(0, subquery_for_set.second.join, columns_added_by_join));
}
