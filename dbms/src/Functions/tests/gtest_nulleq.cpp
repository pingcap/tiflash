...
TEST_F(TestNullEq, TestInputPattern)
try
{
    ...
    auto zero_const_vector = createConstColumn<UInt8>(5, 0);
    auto one_const_vector = createConstColumn<UInt8>(5, 1);
    auto zero_vector = createColumn<UInt8>({0, 0, 0, 0, 0});
    auto one_vector = createColumn<UInt8>({1, 1, 1, 1, 1});

    ColumnsWithTypeAndName first_const_arguments = {
        /// constant: {1, 1, 1, 1, 1}
        createConstColumn<Int64>(5, expr1_data[0]),
        /// nullable(not null constant): {1, 1, 1, 1, 1}
        createConstColumn<Nullable<Int64>>(5, expr1_data[0]),
        /// nullable(null constant): {Null, Null, Null, Null, Null}
        createConstColumn<Nullable<Int64>>(5, {}),
        /// onlyNull constant: {Null, Null, Null, Null, Null}
        createConstColumn<Nullable<Null>>(5, {})};
    ColumnsWithTypeAndName second_const_arguments = {
        /// constant: {5, 5, 5, 5, 5}
        createConstColumn<Int64>(5, expr2_data[0]),
        /// nullable(not null constant): {5, 5, 5, 5, 5}
    ...
    for (auto & col_1 : first_const_arguments)
    {
        for (auto & col_2 : second_const_arguments)
        {
            ...
            {
                ASSERT_COLUMN_EQ(one_const_vector, executeNullEq(col_1, col_2));
            }
    ...
    for (const auto & col_1 : first_vector_arguments)
    {
        for (const auto & col_2 : second_const_arguments)
        {
            ...
            else  /// 1, 2, 3, 4, 5
            {
                ...
                {
                    ASSERT_COLUMN_EQ(createColumn<UInt8>({0, 0, 0, 0, 1}), executeNullEq(col_1, col_2));
                }
            }
        }
    }
