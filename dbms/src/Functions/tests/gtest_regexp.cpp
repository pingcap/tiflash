...
{
    RegexpSubstrCase(Int64 res, const String & expr, const String & pat, Int64 pos = 1, Int64 occur = 1, const String & mt = "")
        : result(res)
        , expression(expr)
        , pattern(pat)
        , position(pos)
    ...
    RegexpSubstrCase(Int64 res, const std::vector<UInt8> & null_map_, const String & expr, const String & pat, Int64 pos = 1, Int64 occur = 1, const String & mt = "")
        : result(res)
        , null_map(null_map_)
        , expression(expr)
        , pattern(pat)
    ...
    static void setVecsWithoutNullMap(int param_num, const std::vector<RegexpSubstrCase> test_cases, std::vector<Int64> & results, std::vector<String> & exprs, std::vector<String> & pats, std::vector<Int64> & positions, std::vector<Int64> & occurs, std::vector<String> & match_types)
    {
        results = getResultVec(test_cases);
        switch (param_num)
        {
    ...
    static void setVecsWithNullMap(int param_num, const std::vector<RegexpSubstrCase> test_cases, std::vector<Int64> & results, std::vector<std::vector<UInt8>> & null_map, std::vector<String> & exprs, std::vector<String> & pats, std::vector<Int64> & positions, std::vector<Int64> & occurs, std::vector<String> & match_types)
    {
        null_map.clear();
...
TEST_F(Regexp, RegexpSubstr)
{
    ...
    // Test: All columns are const
    {
        for (size_t row_size = 1; row_size < 3; ++row_size)
        {
            ASSERT_COLUMN_EQ(createConstColumn<String>(row_size, "123"),
                                executeFunction(
                                    "regexp_substr",
                                    createConstColumn<String>(row_size, "123"),
                                    createConstColumn<String>(row_size, "12.")));
