...
try
{
    ...
    // Invalid Literals
    std::vector<String> invalid_literals = {R"(tRue)", R"(tru)", R"(trued)", R"(False)", R"(fale)", R"(falses)", R"(nulL)", R"(nul)", R"(nulll)"};
    ...
    // Invalid Numbers
    std::vector<String> invalid_numbers = {R"(3.3.3)", R"(3.3t)", R"(--100)", R"(e231.0123)", R"(+3.14e)", R"(-+3.23)", R"(-+341a)"};
    for (const auto & str : invalid_numbers) {
        ASSERT_TRUE(!checkJsonValid(str.c_str(), str.size()));
    }

    // Valid Strings
    std::vector<String> valid_strings = {R"("foo")", R"("hello world!\n")", R"("hello \"name\"")", R"("你好 朋友")", R"("\u554A world")",
        R"("{\"foo\":\"bar\",\"bar\":{\"baz\":[\"qux\"]}}")", R"("\"hello world\"")"};
    for (const auto & str : valid_strings) {
        ASSERT_TRUE(checkJsonValid(str.c_str(), str.size()));
    }

    // Invalid Strings
    std::vector<String> invalid_strings = {R"(""hello world"")", R"("hello world"ef)", R"("hello world)", R"("hello world \ ")"};
    for (const auto & str : invalid_strings) {
        ASSERT_TRUE(!checkJsonValid(str.c_str(), str.size()));
    }

    // Valid Objects
    std::vector<String> valid_objects = {R"({})", R"({"a":3.0, "b":-4, "c":"hello world", "d":true})", R"({"a":3.0, "b":{"c":{"d":"hello world"}}})",
    ...
    // Invalid Objects
    std::vector<String> invalid_objects = {R"({"a")", R"({"a"})", R"({"a":})", R"({32:"a"})", R"({"a":32:})", R"({"a":32}})", R"({"a":32,:})", R"({"a":32,"dd":{"d","e"}})"};
    for (const auto & str : invalid_objects) {
        ASSERT_TRUE(!checkJsonValid(str.c_str(), str.size()));
    }

    // Valid Arrays
    std::vector<String> valid_arrays = {R"([])", R"([true, null, false,  "hello world", 3.0, -1e4])", R"([1,2,[ 3, "hello", ["world", 32]]])", R"([[],[]])"};
    for (const auto & str : valid_arrays) {
        ASSERT_TRUE(checkJsonValid(str.c_str(), str.size()));
    }

    // Invalid Arrays
    std::vector<String> invalid_arrays = {R"([32)", R"([32]])", R"([32,])", R"([[],23)", R"([32], 23)", R"(["hello", ["world"])"};
    for (const auto & str : invalid_arrays) {
        ASSERT_TRUE(!checkJsonValid(str.c_str(), str.size()));
    }

    // Valid Mixtures
    std::vector<String> valid_mixtures = {R"([{}])", R"([3, {"name":3}, {"experince":[3,6,9]}])", R"({"age":3, "value":[-32, true, {"exr":[23,-12,true]}]})"};
    for (const auto & str : valid_mixtures) {
        ASSERT_TRUE(checkJsonValid(str.c_str(), str.size()));
    }

    // Invalid Mixtures
    std::vector<String> invalid_mixtures = {R"({[]})", R"({"name":"tome", [3,2]})", R"([{"name":"tome"})", R"([{"name":"tome"])", R"([{"name":"tome", "age":[}])"};
