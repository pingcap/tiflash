...
struct ToDateTimeImpl
{
    ...
    static inline UInt32 execute(UInt16 d, const DateLUTImpl & time_zone) { return time_zone.fromDayNum(DayNum_t(d)); }
};
...
/** Special case of converting (U)Int32 or (U)Int64 (and also, for convenience, Float32, Float64) to Date.
  ...
  */
template <typename Name>
struct ConvertImpl<DataTypeUInt32, DataTypeDate, Name> : DateTimeTransformImpl<UInt32, UInt16, ToDateTransform32Or64<UInt32, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeUInt64, DataTypeDate, Name> : DateTimeTransformImpl<UInt64, UInt16, ToDateTransform32Or64<UInt64, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeInt32, DataTypeDate, Name> : DateTimeTransformImpl<Int32, UInt16, ToDateTransform32Or64<Int32, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeInt64, DataTypeDate, Name> : DateTimeTransformImpl<Int64, UInt16, ToDateTransform32Or64<Int64, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeFloat32, DataTypeDate, Name> : DateTimeTransformImpl<Float32, UInt16, ToDateTransform32Or64<Float32, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeFloat64, DataTypeDate, Name> : DateTimeTransformImpl<Float64, UInt16, ToDateTransform32Or64<Float64, UInt16>>
{
};


template <typename Name>
struct ConvertImpl<DataTypeMyDateTime, DataTypeMyDate, Name> : DateTimeTransformImpl<UInt64, UInt64, MyDateTimeToMyDateTransform>
{
};

template <typename Name>
struct ConvertImpl<DataTypeUInt32, DataTypeMyDate, Name> : DateTimeTransformImpl<UInt32, UInt64, ToMyDateTransform32Or64<UInt32, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeUInt64, DataTypeMyDate, Name> : DateTimeTransformImpl<UInt64, UInt64, ToMyDateTransform32Or64<UInt64, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeInt32, DataTypeMyDate, Name> : DateTimeTransformImpl<Int32, UInt64, ToMyDateTransform32Or64<Int32, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeInt64, DataTypeMyDate, Name> : DateTimeTransformImpl<Int64, UInt64, ToMyDateTransform32Or64<Int64, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeFloat32, DataTypeMyDate, Name> : DateTimeTransformImpl<Float32, UInt64, ToMyDateTransform32Or64<Float32, UInt16>>
{
};
template <typename Name>
struct ConvertImpl<DataTypeFloat64, DataTypeMyDate, Name> : DateTimeTransformImpl<Float64, UInt64, ToMyDateTransform32Or64<Float64, UInt16>>
{
};


template <typename Name>
struct ConvertImpl<DataTypeMyDate, DataTypeMyDateTime, Name> : DateTimeTransformImpl<UInt64, UInt64, MyDateToMyDateTimeTransform>
{
};
...
struct FormatImpl
{
    static void execute(const typename DataType::FieldType x, WriteBuffer & wb, const DataType *, const DateLUTImpl *) { writeText(x, wb); }
};
...
{
    static void execute(const typename DataTypeDecimal<DecimalType>::FieldType v, WriteBuffer & wb, const DataTypeDecimal<DecimalType> * tp,
        const DateLUTImpl *)
    {
...
template <typename FromDataType, typename ToDataType, typename Name, ConvertFromStringExceptionMode exception_mode,
...
struct ConvertThroughParsing
{
    ...
    static bool isAllRead(ReadBuffer & in)
    {
        ...
        if ((std::is_same_v<ToDataType, DataTypeDate> || std::is_same_v<ToDataType, DataTypeMyDate>)&&(in.buffer().size())
...
template <>
struct ConvertImpl<DataTypeString, DataTypeUInt32, NameToUnixTimestamp> : ConvertImpl<DataTypeString, DataTypeDateTime, NameToUnixTimestamp>
{
};
...
public:
    ...
    void executeImpl(Block & block, const ColumnNumbers & arguments, size_t result) override
    {
        ...
        catch (Exception & e)
        {
            ...
            {
                e.addMessage("Cannot parse " + block.getByPosition(result).type->getName() + " from "
                    + block.getByPosition(arguments[0]).type->getName() + ", because value is too short");
            }
            else if (e.code() == ErrorCodes::CANNOT_PARSE_NUMBER || e.code() == ErrorCodes::CANNOT_READ_ARRAY_FROM_TEXT
...
private:
    ...
    {
        if (!arguments.size())
            throw Exception{"Function " + getName() + " expects at least 1 arguments", ErrorCodes::TOO_LESS_ARGUMENTS_FOR_FUNCTION};
...
/** Function toTOrZero (where T is number of date or datetime type):
  ...
  */
template <typename ToDataType, typename Name, ConvertFromStringExceptionMode exception_mode,
    ...
    void executeImpl(Block & block, const ColumnNumbers & arguments, size_t result) override
    {
        ...
        if (checkAndGetDataType<DataTypeString>(from_type))
            ...
            throw Exception("Illegal type " + block.getByPosition(arguments[0]).type->getName() + " of argument of function " + getName()
                    + ". Only String or FixedString argument is accepted for try-conversion function. For other arguments, use function "
                      "without 'orZero' or 'orNull'.",
                ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT);
    }
};
...
public:
    ...
    void executeImpl(Block & block, const ColumnNumbers & arguments, const size_t result) override
    {
        ...
        else
        {
            ...
            Block temporary_block{{ColumnNullable::create(std::move(datetime_column), std::move(null_column)),
                                      makeNullable(std::make_shared<DataTypeMyDateTime>(std::min<UInt32>(scale, 6))), ""},
                block.getByPosition(arguments[1]), {nullptr, makeNullable(std::make_shared<DataTypeString>()), ""}};
...
public:
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        if (!removeNullable(arguments[1].type)->isString())
            throw Exception("Second argument for function " + getName() + " must be String, but get " + arguments[1].type->getName(),
                ErrorCodes::ILLEGAL_COLUMN);
...
struct ToIntMonotonicity
{
    ...
    static IFunction::Monotonicity get(const IDataType & type, const Field & left, const Field & right)
    {
        ...
        if ((left.get<Int64>() >= 0) != (right.get<Int64>() >= 0) || (T(left.get<Int64>()) >= 0) != (T(right.get<Int64>()) >= 0))
            return {};
...
using FunctionParseDateTimeBestEffort = FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffort,
    ConvertFromStringExceptionMode::Throw, ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTimeBestEffortOrZero = FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortOrZero,
    ConvertFromStringExceptionMode::Zero, ConvertFromStringParsingMode::BestEffort>;
using FunctionParseDateTimeBestEffortOrNull = FunctionConvertFromString<DataTypeDateTime, NameParseDateTimeBestEffortOrNull,
    ConvertFromStringExceptionMode::Null, ConvertFromStringParsingMode::BestEffort>;
...
private:
    ...
    WrapperType createWrapper(const DataTypePtr & from_type, const DataType * const) const
    {
        ...
        return [function](
                   Block & block, const ColumnNumbers & arguments, const size_t result) { function->execute(block, arguments, result); };
    }
    ...
    WrapperType createArrayWrapper(const DataTypePtr & from_type_untyped, const DataTypeArray * to_type) const
    {
        ...
        if ((from_type == nullptr) != (to_type == nullptr))
            throw Exception{
                "CAST AS Array can only be performed between same-dimensional array types or from String", ErrorCodes::TYPE_MISMATCH};

        /// Prepare nested type conversion
        const auto nested_function = prepare(from_nested_type, to_nested_type);

        return [nested_function, from_nested_type, to_nested_type](Block & block, const ColumnNumbers & arguments, const size_t result) {
    ...
    WrapperType createTupleWrapper(const DataTypePtr & from_type_untyped, const DataTypeTuple * to_type) const
    {
        ...
        return
            [element_wrappers, from_element_types, to_element_types](Block & block, const ColumnNumbers & arguments, const size_t result) {
    ...
    void checkEnumToEnumConversion(const EnumTypeFrom * from_type, const EnumTypeTo * to_type) const
    {
        ...
        std::set_intersection(std::begin(from_values), std::end(from_values), std::begin(to_values), std::end(to_values),
            std::back_inserter(name_intersection), [](auto && from, auto && to) { return from.first < to.first; });
        ...
        for (const auto & name_value : name_intersection)
        {
            ...
            if (old_value != new_value)
                throw Exception{"Enum conversion changes value for element '" + name_value.first + "' from " + toString(old_value) + " to "
                        + toString(new_value),
                    ErrorCodes::CANNOT_CONVERT_TYPE};
        }
    };
        ...
        else if (nullable_conversion.source_is_nullable)
        {
            ...
            return [wrapper](Block & block, const ColumnNumbers & arguments, const size_t result) {
                ...
                if (!memoryIsZero(null_map.data(), null_map.size()))
                    throw Exception{"Cannot convert NULL value to non-Nullable type", ErrorCodes::CANNOT_INSERT_NULL_IN_ORDINARY_COLUMN};
