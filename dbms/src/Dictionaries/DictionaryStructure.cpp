// Modified from: https://github.com/ClickHouse/ClickHouse/blob/30fcaeb2a3fff1bf894aae9c776bed7fd83f783f/dbms/src/Dictionaries/DictionaryStructure.cpp
...
DictionarySpecialAttribute::DictionarySpecialAttribute(
...
DictionaryStructure::DictionaryStructure(
    const Poco::Util::AbstractConfiguration & config,
    const std::string & config_prefix)
{
    ...
    if (id)
    {
        ...
        if (!id->expression.empty() || (range_min && !range_min->expression.empty())
...
void DictionaryStructure::validateKeyTypes(const DataTypes & key_types) const
{
    ...
    for (const auto i : ext::range(0, key_types.size()))
    {
        ...
        if (expected_type != actual_type)
            throw Exception{
                "Key type at position " + std::to_string(i) + " does not match, expected " + expected_type + ", found "
                    + actual_type,
