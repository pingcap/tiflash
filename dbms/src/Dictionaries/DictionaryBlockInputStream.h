...
private:
    ...
    using DictionaryGetter
        = void (DictionaryType::*)(const std::string &, const PaddedPODArray<Key> &, PaddedPODArray<Type> &) const;
    using DictionaryStringGetter
        = void (DictionaryType::*)(const std::string &, const PaddedPODArray<Key> &, ColumnString *) const;
    // for complex complex key dictionaries
    template <typename Type>
    using GetterByKey = void (
        DictionaryType::*)(const std::string &, const Columns &, const DataTypes &, PaddedPODArray<Type> & out) const;
    using StringGetterByKey
        = void (DictionaryType::*)(const std::string &, const Columns &, const DataTypes &, ColumnString * out) const;
...
DictionaryBlockInputStream<DictionaryType, Key>::DictionaryBlockInputStream(
    ...
    , fillBlockFunction(
          &DictionaryBlockInputStream<DictionaryType, Key>::fillBlock<DictionaryGetter, DictionaryStringGetter>)
    ...
    ColumnsWithTypeAndName & res) const
{
    ...
    for (size_t i = 0, size = columns.size(); i < size; ++i)
        res.emplace_back(ColumnWithTypeAndName{
            std::move(columns[i]),
            (*dictionary_structure.key)[i].type,
            (*dictionary_structure.key)[i].name});
}
