// Modified from: https://github.com/ClickHouse/ClickHouse/blob/30fcaeb2a3fff1bf894aae9c776bed7fd83f783f/dbms/src/Dictionaries/TrieDictionary.cpp
    ...
    DefaultGetter && get_default) const
{
    ...
    if (first_column->isNumeric())
    {
        ...
        {
            auto addr = static_cast<Int32>(first_column->get64(i));
...
bool TrieDictionary::setAttributeValueImpl(Attribute & attribute, const StringRef key, const T value)
{
    ...
    const auto * addr_v6 = reinterpret_cast<const uint8_t *>(addr.addr());
    const auto * mask_v6 = reinterpret_cast<const uint8_t *>(mask.addr());
...
void TrieDictionary::trieTraverse(const btrie_t * tree, Getter && getter) const
{
    ...
    auto get_bit = [&high_bit](size_t size) {
...
Columns TrieDictionary::getKeyColumns() const
{
    ...
    auto getter = [&ip_column, &mask_column](__uint128_t ip, size_t mask) {
        auto * ip_array = reinterpret_cast<UInt64 *>(&ip);
