...
HashedDictionary::HashedDictionary(const std::string & name, const DictionaryStructure & dict_struct, DictionarySourcePtr source_ptr, const DictionaryLifetime dict_lifetime, bool require_nonempty, BlockPtr saved_block)
    : name{name}
...
void HashedDictionary::updateData()
{
    ...
    else
    {
        ...
        while (const auto block = stream->read())
        {
            ...
            robin_hood::unordered_map<Key, std::vector<size_t>> update_ids;
            ...
            robin_hood::unordered_map<Key, std::vector<size_t>>::iterator it;
