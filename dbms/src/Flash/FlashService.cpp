...
grpc::Status FlashService::CoprocessorStream(grpc::ServerContext * grpc_context,
                                             const coprocessor::Request * request,
                                             grpc::ServerWriter<coprocessor::Response> * writer)
{
    ...
    bool is_remote_read = getClientMetaVarWithDefault(grpc_context, "is_remote_read", "") == "true";
    auto log_level = is_remote_read ? Poco::Message::PRIO_INFORMATION : Poco::Message::PRIO_DEBUG;
    LOG_IMPL(log, log_level, "Handling coprocessor stream request, is_remote_read: {}, start ts: {}, region info: {}, region epoch: {}", is_remote_read, request->start_ts(), request->context().region_id(), request->context().region_epoch().DebugString());
    ...
    if (handle_limit > 0)
    {
        // We use this atomic variable metrics from the prometheus-cpp library to mark the number of queued queries.
        // TODO: Use grpc asynchronous server and a more fully-featured thread pool.
        if (auto current = GET_METRIC(tiflash_coprocessor_handling_request_count, type_cop_stream).Value(); current > handle_limit)
        {
            coprocessor::Response response;
            response.mutable_region_error()->mutable_server_is_busy()->set_reason(fmt::format("tiflash cop stream pool queued too much, current = {}, limit = {}", current, handle_limit));
    ...
    grpc::Status ret = executeInThreadPool(*cop_stream_pool, [&] {
        ...
        if (max_queued_duration_ms > 0)
        {
            ...
            {
                coprocessor::Response response;
                response.mutable_region_error()->mutable_server_is_busy()->set_reason(fmt::format("this task queued in tiflash cop stream pool too long, current = {} ms, limit = {} ms", wait_ms, max_queued_duration_ms));
                writer->Write(response);
                return grpc::Status::OK;
            }
        }
        if (wait_ms > 1000)
            log_level = Poco::Message::PRIO_INFORMATION;
        LOG_IMPL(log, log_level, "Begin process cop stream request after wait {} ms, start ts: {}, region info: {}, region epoch: {}", wait_ms, request->start_ts(), request->context().region_id(), request->context().region_epoch().DebugString());
        auto [db_context, status] = createDBContext(grpc_context);
        if (!status.ok())
        {
            return status;
