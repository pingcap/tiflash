...
bool needRemoteRead(const RegionInfo & region_info, const TiFlashContext & flash_ctx)
{
    fiu_do_on(FailPoints::force_no_local_region_for_mpp_task, { return true; });
    RegionPtr current_region = flash_ctx.getKVStore()->getRegion(region_info.region_id);
...
void MPPTask::prepare(const mpp::DispatchTaskRequest & task_request)
{
    ...
    for (const auto & r : task_request.regions())
    {
        ...
        if (duplicated_region || needRemoteRead(region_info, flash_ctx))
