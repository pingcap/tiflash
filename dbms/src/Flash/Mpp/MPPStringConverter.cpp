...
NamesAndTypes MPPStringConverter::buildTSString(const tipb::TableScan & ts, std::stringstream & ss)
{
    TableID table_id;
    if (ts.has_table_id())
    {
...
NamesAndTypes MPPStringConverter::buildSelString(const tipb::Selection & sel, std::stringstream & ss)
{
    auto input_column = buildString(sel.child(), ss);
    String child_str = ss.str();
    ss.clear();
    ss << genPrefixString() << "selection( conditions: ";
    exprsToString(sel.conditions(), input_column, ss);
    ss << " )\n" << child_str;
    return input_column;
}

NamesAndTypes MPPStringConverter::buildLimitString(const tipb::Limit & limit, std::stringstream & ss)
{
    auto input_column = buildString(limit.child(), ss);
    String child_str = ss.str();
    ss.clear();
    auto limit_count = limit.limit();
    ss << genPrefixString() << "limit( limit_count: " << limit_count << " )\n" << child_str;
    return input_column;
}

NamesAndTypes MPPStringConverter::buildProjString(const tipb::Projection & proj, std::stringstream & ss)
{
    ...
    ss << genPrefixString() << "projection( [";
    namesAndTypesToString(columns_from_proj, ss);
    ss << "] )\n" << child_str;
    return columns_from_proj;
}

NamesAndTypes MPPStringConverter::buildAggString(const tipb::Aggregation & agg, std::stringstream & ss)
{
    ...
    ss << genPrefixString() << "aggregation( agg_funcs: [";
...
NamesAndTypes MPPStringConverter::buildTopNString(const tipb::TopN & topN, std::stringstream & ss)
{
    auto input_column = buildString(topN.child(), ss);
    String child_str = ss.str();
    ss.clear();
    ss << genPrefixString() << "top_n( order by: [";
    byItemsToString(topN.order_by(), input_column, ss);
    ss << "] )\n" << child_str;
    return input_column;
}

NamesAndTypes MPPStringConverter::buildJoinString(const tipb::Join & join, std::stringstream & ss)
{
    if (join.children_size() != 2)
        throw TiFlashException("Join executor children size not equal to 2", Errors::Coprocessor::BadRequest);
    auto input_column = buildString(join.children(0), ss);
    String left_child_str = ss.str();
    ss.clear();
    auto right_input_column = buildString(join.children(1), ss);
    String right_child_str = ss.str();
    ss.clear();
    input_column.insert(input_column.end(), right_input_column.cbegin(), right_input_column.cend());

    ss << genPrefixString() << "join( left_conditions: [";
    exprsToString(join.left_conditions(), input_column, ss);
    ss << "] right_conditions: [";
    exprsToString(join.right_conditions(), input_column, ss);
    ss << "] other_conditions: [";
    exprsToString(join.other_conditions(), input_column, ss);
    ss << "] )\n" << left_child_str << '\n' << right_child_str;
    return input_column;
}

std::vector<NameAndTypePair> MPPStringConverter::buildExchangeSenderString(const tipb::ExchangeSender & exchange_sender, std::stringstream & ss)
{
    auto input_column = buildString(exchange_sender.child(), ss);
    String child_str = ss.str();
    ss.clear();
    ss << genPrefixString() << "exchange_sender\n" << child_str;
    return input_column;
}

NamesAndTypes MPPStringConverter::buildExchangeReceiverString(const tipb::ExchangeReceiver & exchange_receiver, std::stringstream & ss)
{
    ...
    ss << genPrefixString() << "exchange_receiver( ";
...
struct CurrentLevelCounter
{
    ...
    explicit CurrentLevelCounter(size_t & level) : current_level(level)
    {
        ++current_level;
    }
...
NamesAndTypes MPPStringConverter::buildString(const tipb::Executor & executor, std::stringstream & ss)
{
    ...
    switch (executor.tp())
    {
        ...
        case tipb::ExecType::TypeJoin:
            return buildJoinString(executor.join(), ss);
        ...
        case tipb::ExecType::TypeExchangeSender:
            return buildExchangeSenderString(executor.exchange_sender(), ss);
        case tipb::ExecType::TypeExchangeReceiver:
            return buildExchangeReceiverString(executor.exchange_receiver(), ss);
