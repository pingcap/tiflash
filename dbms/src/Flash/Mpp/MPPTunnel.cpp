...
void MPPTunnel::write(const mpp::MPPDataPacket & data, bool close_after_write)
{
    LOG_FMT_TRACE(log, "ready to write");
    {
        ...
        if (send_queue->push(std::make_shared<mpp::MPPDataPacket>(data)) == MPMCQueueStatus::NORMAL)
        {
...
void SyncTunnelSender::sendJob()
{
    ...
    try
    {
        ...
        while (send_queue->pop(res) == MPMCQueueStatus::NORMAL)
        {
            if (!writer->write(*res))
            {
...
void AsyncTunnelSender::sendOne(bool use_lock)
{
    ...
    {
        MPPDataPacketPtr res;
        queue_empty_flag = send_queue->pop(res) != MPMCQueueStatus::NORMAL;
        if (!queue_empty_flag)
        {
            if (!writer->write(*res))
            {
...
{
    MPPDataPacketPtr res;
    if (send_queue->pop(res) == MPMCQueueStatus::NORMAL)
        return res;
    consumerFinish("");
    return nullptr;
}
