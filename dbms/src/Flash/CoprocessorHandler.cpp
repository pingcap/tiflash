...
grpc::Status CoprocessorHandler::execute()
{
    ...
    try
    {
        ...
        case COP_REQ_TYPE_DAG:
        {
            ...
            LOG_DEBUG(logger, "Handling DAG request: {}", dag_request.DebugString());
            ...
            LOG_DEBUG(logger, "Handle DAG request done");
            break;
        }
    ...
    {
        LOG_WARNING(logger, "LockException: region {}, message: {}", cop_request->context().region_id(), e.message());
        cop_response->Clear();
    ...
    {
        LOG_WARNING(logger, "RegionException: region {}, message: {}", cop_request->context().region_id(), e.message());
        cop_response->Clear();
    ...
    {
        LOG_ERROR(logger, "KV Client Exception: {}", e.message());
    ...
    {
        LOG_ERROR(logger, "other exception");
        GET_METRIC(tiflash_coprocessor_request_error, reason_other_error).Increment();
