...
CoprocessorHandler::CoprocessorHandler(
    ...
    , logger(Logger::get("CoprocessorHandler"))
{}
...
grpc::Status CoprocessorHandler::execute()
{
    ...
    try
    {
        ...
        case COP_REQ_TYPE_DAG:
        {
            ...
            LOG_FMT_DEBUG(logger, "Handling DAG request: {}", dag_request.DebugString());
            ...
            LOG_FMT_DEBUG(logger, "Handle DAG request done");
            break;
        }
    ...
    {
        LOG_FMT_WARNING(logger, "LockException: region {}, message: {}", cop_request->context().region_id(), e.message());
        cop_response->Clear();
    ...
    {
        LOG_FMT_WARNING(logger, "RegionException: region {}, message: {}", cop_request->context().region_id(), e.message());
        cop_response->Clear();
    ...
    {
        LOG_FMT_ERROR(logger, "KV Client Exception: {}", e.message());
    ...
    {
        LOG_FMT_ERROR(logger, "other exception");
        GET_METRIC(tiflash_coprocessor_request_error, reason_other_error).Increment();
