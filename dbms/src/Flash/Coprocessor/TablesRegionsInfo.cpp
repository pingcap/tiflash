...
TableRegionsInfo & TablesRegionsInfo::getOrCreateTableRegionInfoByTableID(Int64 table_id)
{
    if (is_single_table)
        return table_regions_info_map.begin()->second;
    if (table_regions_info_map.find(table_id) == table_regions_info_map.end())
    {
        table_regions_info_map[table_id] = TableRegionsInfo();
    }
    return table_regions_info_map.find(table_id)->second;
}
const TableRegionsInfo & TablesRegionsInfo::getTableRegionInfoByTableID(Int64 table_id) const
{
    if (is_single_table)
        return table_regions_info_map.begin()->second;
    if (table_regions_info_map.find(table_id) != table_regions_info_map.end())
        return table_regions_info_map.find(table_id)->second;
    throw Exception("Can't find region info for table id " + std::to_string(table_id));
}
...
void insertRegionInfoToTablesRegionInfo(const google::protobuf::RepeatedPtrField<coprocessor::RegionInfo> & regions, Int64 table_id, TablesRegionsInfo & tables_region_infos, std::unordered_set<RegionID> & local_region_id_set, const TMTContext & tmt_context)
{
...
TablesRegionsInfo TablesRegionsInfo::createTablesRegionsInfo(const google::protobuf::RepeatedPtrField<coprocessor::RegionInfo> & regions,
