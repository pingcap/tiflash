...
MakeRegionQueryInfos(
    const std::unordered_map<Int64, std::reference_wrapper<const RegionInfoMap>> & dag_region_infos,
    ...
    for (const auto & [physical_table_id, regions] : dag_region_infos)
    {
        for (const auto & [id, r] : regions.get())
        {
            ...
            RegionQueryInfo info;
            {
                ...
                info.physical_table_id = physical_table_id;
...
LearnerReadSnapshot DAGStorageInterpreter::doCopLearnerRead()
{
    ...
    std::unordered_map<Int64, std::reference_wrapper<const RegionInfoMap>> regions_for_local_read;
    ...
    return doLearnerRead(logical_table_id, *mvcc_query_info, max_streams, /*wait_index_timeout_as_region_not_found*/ true, context, log);
}

/// Will assign region_retry
LearnerReadSnapshot DAGStorageInterpreter::doBatchCopLearnerRead()
{
    std::unordered_map<TableID, std::reference_wrapper<const RegionInfoMap>> regions_for_local_read;
    for (const auto physical_table_id : table_scan.getPhysicalTableIDs())
    {
    ...
    for (;;)
    {
        try
        {
            ...
            if (mvcc_query_info->regions_query_info.empty())
                return {};
            return doLearnerRead(logical_table_id, *mvcc_query_info, max_streams, /*wait_index_timeout_as_region_not_found*/ false, context, log);
        }
...
{
    std::vector<ManageableStoragePtr> storages_;
    std::vector<TableStructureLockHolder> locks_;
    std::unordered_map<TableID, DAGStorageInterpreter::StorageWithStructureLock> storages_with_lock_;
    if (unlikely(query_schema_version == DEFAULT_UNSPECIFIED_SCHEMA_VERSION))
    {
        ...
        storages_with_lock_[logical_table_id] = {logical_table_storage, logical_table_storage->lockStructureForShare(context.getCurrentQueryId())};
        if (table_scan.isPartitionTableScan())
        {
            for (auto const physical_table_id : table_scan.getPhysicalTableIDs())
            {
                ...
                storages_with_lock_[physical_table_id] = {physical_table_storage, physical_table_storage->lockStructureForShare(context.getCurrentQueryId())};
            }
        }
        return storages_with_lock_;
    }
    ...
    auto get_and_lock_storage = [&](bool schema_synced, TableID table_id) -> std::tuple<ManageableStoragePtr, TableStructureLockHolder, Int64, bool> {
        ...
        if (table_store->engineType() != ::TiDB::StorageEngine::TMT && table_store->engineType() != ::TiDB::StorageEngine::DT)
        {
            throw TiFlashException("Specifying schema_version for non-managed storage: " + table_store->getName()
                                       + ", table: " + table_store->getTableName() + ", id: " + DB::toString(logical_table_id) + " is not allowed",
        ...
        if (storage_schema_version > query_schema_version)
            throw TiFlashException("Table " + std::to_string(logical_table_id) + " schema version " + std::to_string(storage_schema_version)
    ...
    auto get_and_lock_storages = [&](bool schema_synced) -> std::tuple<std::vector<ManageableStoragePtr>, std::vector<TableStructureLockHolder>, std::vector<Int64>, bool> {
        ...
        bool meet_unmatched_schema = false;
        for (auto const physical_table_id : table_scan.getPhysicalTableIDs())
        {
    ...
    auto log_schema_version = [&](const String & result, std::vector<Int64> storage_schema_versions) {
        FmtBuffer buffer;
        buffer.fmtAppend("{} Table {} schema {} Schema version [storage, global, query]: [{}, {}, {}]", __PRETTY_FUNCTION__, logical_table_id, result, storage_schema_versions.empty() ? DEFAULT_UNSPECIFIED_SCHEMA_VERSION : storage_schema_versions[0], global_schema_version, query_schema_version);
        if (table_scan.isPartitionTableScan())
        {
            for (size_t i = 0; i < table_scan.getPhysicalTableIDs().size(); i++)
            {
                const auto physical_table_id = table_scan.getPhysicalTableIDs()[i];
                buffer.fmtAppend(", Table {} schema {} Schema version [storage, global, query]: [{}, {}, {}]", physical_table_id, result, storage_schema_versions.empty() ? DEFAULT_UNSPECIFIED_SCHEMA_VERSION : storage_schema_versions[1 + i], global_schema_version, query_schema_version);
            }
        }
        LOG_INFO(log, buffer.toString());
    };
    ...
    if (ok)
    {
        log_schema_version("OK, no syncing required.", storage_schema_versions);
        //return {storages, locks};
    }
    else
    /// If first try failed, sync schema and try again.
    {
        ...
        if (ok)
        {
            log_schema_version("OK after syncing.", storage_schema_versions);
        }
        else
    ...
    for (size_t i = 0; i < storages.size(); i++)
    {
        ...
        storages_with_lock_[table_id] = {std::move(storages[i]), std::move(locks[i])};
    }
    return storages_with_lock_;
}
...
std::tuple<Names, NamesAndTypes, std::vector<ExtraCastAfterTSMode>, String> DAGStorageInterpreter::getColumnsForTableScan(Int64 max_columns_to_read)
{
    ...
    for (Int32 i = 0; i < table_scan.getColumnSize(); i++)
    {
        ...
        String name = cid == -1 ? handle_column_name_ : storage_for_logical_table->getTableInfo().getColumnName(cid);
...
void DAGStorageInterpreter::buildRemoteRequest()
{
    ...
    for (const auto physical_table_id : table_scan.getPhysicalTableIDs())
    {
        ...
        auto print_retry_regions = [&retry_regions, &physical_table_id] {
            FmtBuffer buffer;
            buffer.fmtAppend("Start to retry {} regions (", retry_regions.size());
            buffer.joinStr(
                retry_regions.cbegin(),
                retry_regions.cend(),
                [](const auto & r, FmtBuffer & fb) { fb.fmtAppend("{}", r.get().region_id); },
                ",");
            buffer.fmtAppend(") for table {}", physical_table_id);
        ...
        {
            auto & storage = storages_with_structure_lock[physical_table_id].storage;
            ...
            table_scan.constructTableScanForRemoteRead(mutable_table_scan, physical_table_id);
        ...
        remote_requests.emplace_back(std::move(dag_req), std::move(schema), std::move(key_ranges));
    }
}
