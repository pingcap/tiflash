...
DAGStorageInterpreter::DAGStorageInterpreter(
    ...
    size_t max_streams_)
    : context(context_)
    , table_scan_executor_id(table_scan_executor_id_)
    , table_scan(table_scan_)
    , max_streams(max_streams_)
    , log(getMPPTaskLog(*context.getDAGContext(), "DAGStorageInterpreter"))
    , table_id(table_scan_.table_id())
    ...
    const ManageableStoragePtr & storage)
{
    ...
    return {required_columns, source_columns, need_cast_column, handle_column_name};
}
    ...
    const String & handle_column_name)
{
...

    {
        ...
        ts_exec->set_executor_id(table_scan_executor_id);
