...
LearnerReadSnapshot DAGStorageInterpreter::doCopLearnerRead()
{
    ...
    return doLearnerRead(table_id, *mvcc_query_info, max_streams, /*is_batch_cop*/ false, tmt, log->getLog());
}
...
LearnerReadSnapshot DAGStorageInterpreter::doBatchCopLearnerRead()
{
    ...
    for (;;)
    {
        try
        {
            ...
            if (mvcc_query_info->regions_query_info.empty())
                return {};
            return doLearnerRead(table_id, *mvcc_query_info, max_streams, /*is_batch_cop*/ true, tmt, log->getLog());
        }
