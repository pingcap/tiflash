...
void DAGResponseWriter::addExecuteSummaries(tipb::SelectResponse & response, bool delta_mode)
{
    ...
    for (auto & p : dag_context.getProfileStreamsMap())
    {
        ...
        for (auto & join_executor_id : dag_context.getSourceNameToJoinExecutorIdMap()[p.first])
        {
            UInt64 process_time_for_build = 0;
            if (dag_context.getProfileStreamsMapForJoinBuildSide().find(join_executor_id)
                != dag_context.getProfileStreamsMapForJoinBuildSide().end())
            {
                for (auto & join_stream : dag_context.getProfileStreamsMapForJoinBuildSide()[join_executor_id])
                {
