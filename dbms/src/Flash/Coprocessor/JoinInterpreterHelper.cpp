    ...
    const ExpressionActionsPtr & probe_prepare_join_actions) const
{
    ...
    append_origin_columns(left_cols, shouldMakeLeftJoinSideNullable(join.join_type()));
    append_origin_columns(right_cols, shouldMakeRightJoinSideNullable(join.join_type()));
    ...
    bool make_nullable = build_side_index == 1 ? shouldMakeLeftJoinSideNullable(join.join_type())
                                               : shouldMakeRightJoinSideNullable(join.join_type());
    ...
    const String & match_helper_name) const
{
    ...
    append_output_columns(left_cols, shouldMakeLeftJoinSideNullable(join.join_type()));
    if (!isSemiFamily() && !isLeftOuterSemiFamily())
    {
        /// for (left outer) semi join, the columns from right table will be ignored
        append_output_columns(right_cols, shouldMakeRightJoinSideNullable(join.join_type()));
    }
