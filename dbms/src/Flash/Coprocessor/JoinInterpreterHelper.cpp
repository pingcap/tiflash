    ...
    const ExpressionActionsPtr & prepare_join_actions2) const
{
    ...
    /// todo simplify logic: just combine the output columns of prepare_join_actions1 and prepare_join_actions2
    ...
    auto append_new_columns = [&columns_for_other_join_filter, &column_set_for_other_join_filter](const Block & header) {
        for (auto const & p : header)
        {
    ...
    append_new_columns(prepare_join_actions1->getSampleBlock());
    append_new_columns(prepare_join_actions2->getSampleBlock());
