    ...
    const ExpressionActionsPtr & right_prepare_join_actions) const
{
    ...
    RUNTIME_ASSERT(is_prepare_actions_valid(left_input_header, left_prepare_join_actions), "left_prepare_join_actions isn't valid");
    RUNTIME_ASSERT(is_prepare_actions_valid(right_input_header, right_prepare_join_actions), "right_prepare_join_actions isn't valid");
#endif
    ...
    /// append the columns generated by prepare join.
    /// the new columns are
    ...
    /// because the input that other filter accepts is {left_input_columns, right_input_columns, new_columns_from_probe_side_prepare, match_helper_name}
    append_new_columns(left_prepare_join_actions->getSampleBlock(), join.join_type() == tipb::JoinType::TypeRightOuterJoin);
    append_new_columns(right_prepare_join_actions->getSampleBlock(), join.join_type() == tipb::JoinType::TypeLeftOuterJoin);
