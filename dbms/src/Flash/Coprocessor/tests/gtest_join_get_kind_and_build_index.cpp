...
TEST(JoinKindAndBuildIndexTestRunner, TestFullJoinAllowsLeftAndRightConditions)
{
    ...
    ASSERT_STREQ(
        left_only_conditions.validate(ASTTableJoin::Kind::Inner),
        "left conditions only supported for left outer and full outer join");
    ...
    ASSERT_STREQ(
        right_only_conditions.validate(ASTTableJoin::Kind::Inner),
        "right conditions only supported for right outer and full outer join");
}
