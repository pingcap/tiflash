    ...
    TiDBColumn & dag_column, const IColumn * flash_col_untyped, size_t start_index, size_t end_index, const IDataType * data_type)
{
    ...
    for (size_t i = start_index; i < end_index; i++)
    {
        ...
        if (enum_value == 0 || enum_value > enum_value_size)
            throw TiFlashException("number of enum overflow enum boundary", TiFlashErrorRegistry::simpleGet("Coprocessor", "Internal"));
    ...
    size_t start_index, size_t end_index)
{
    ...
    if (type->isNullable() && tidb_column_info.hasNotNullFlag())
    {
        throw TiFlashException(
            "Flash column and TiDB column has different not null flag", TiFlashErrorRegistry::simpleGet("Coprocessor", "Internal"));
    }
    if (type->isNullable())
    ...
    switch (tidb_column_info.tp)
    {
        ...
        case TiDB::TypeTime:
            ...
            if (type->isUnsignedInteger() != tidb_column_info.hasUnsignedFlag())
                throw TiFlashException(
                    "Flash column and TiDB column has different unsigned flag", TiFlashErrorRegistry::simpleGet("Coprocessor", "Internal"));
    ...
    const std::vector<UInt64> & offsets, const ColumnWithTypeAndName & flash_col, const ColumnInfo & col_info, UInt32 length)
{
    switch (col_info.tp)
    {
        ...
        default:
            throw TiFlashException(
                "Not supported yet: field tp = " + std::to_string(col_info.tp), TiFlashErrorRegistry::simpleGet("Coprocessor", "Unimplemented"));
    }
}
