    ...
    const std::unordered_set<RegionID> & region_force_retry, TMTContext & tmt, MvccQueryInfo & mvcc_info, TableID table_id)
{
    ...
    for (auto & [id, r] : dag_region_infos)
    {
        ...
        {
            throw TiFlashException("Income key ranges is empty for region: " + std::to_string(r.region_id), TiFlashErrorRegistry::simpleGet("Coprocessor", "BadRequest"));
        }
            ...
            for (const auto & p : r.key_ranges)
            {
                ...
                if (range.first < info.range_in_table.first || range.second > info.range_in_table.second)
                    throw TiFlashException(
                        "Income key ranges is illegal for region: " + std::to_string(r.region_id), TiFlashErrorRegistry::simpleGet("Coprocessor", "BadRequest"));
