...
void PlainPipelineEvent::scheduleImpl()
{
    ...
    for (auto & pipeline_exec : pipeline_exec_group)
        addTask(std::make_unique<PipelineTask>(exec_context, log->identifier(), shared_from_this(), std::move(pipeline_exec), context.getDAGContext()->getResourceGroupName(), context.getDAGContext()->getKeyspaceID()));
}
