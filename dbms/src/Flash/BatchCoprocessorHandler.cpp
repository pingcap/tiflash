...
grpc::Status BatchCoprocessorHandler::execute()
{
    ...
    try
    {
        ...
        case COP_REQ_TYPE_DAG:
        {
            ...
            assert(cop_request->regions_size() == 0 || cop_request->table_regions_size() == 0);
            auto dag_request = getDAGRequestFromStringWithRetry(cop_request->data());
            TablesRegionsInfo tables_regions_info = TablesRegionsInfo::createTablesRegionsInfo(cop_request->regions(), cop_request->table_regions(), cop_context.db_context.getTMTContext());
            LOG_FMT_DEBUG(
                log,
