    ...
    std::call_once(task_pool->addToSchedulerFlag(), [&]() {
        ...
        else
        {
            ...
            else
            {
                ...
                TaskScheduler::instance->submitToWaitReactor(std::make_unique<RFWaitTask>(exec_status, log->identifier(), task_pool, max_wait_time_ms, std::move(waiting_rf_list), std::move(ready_rf_list)));
            }
        }
    });
}
