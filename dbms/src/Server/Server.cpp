...
struct RaftStoreProxyRunner : boost::noncopyable
{
    ...
    void run()
    {
        ...
        LOG_FMT_INFO(log, "start raft store proxy with params: {}", parms.conf.val_map);
...
{
public:
    FlashGrpcServerHolder(Server & server, const TiFlashRaftConfig & raft_config, const LoggerPtr & log_)
        : log(log_)
