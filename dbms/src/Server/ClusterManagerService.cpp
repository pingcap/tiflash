...
const std::string CLUSTER_MANAGER_PATH_KEY = "flash.cluster_manager_path";
const std::string BIN_NAME = "flash_cluster_manager";
const std::string TASK_INTERVAL_KEY = "flash.flash_cluster.refresh_interval";
    ...
    : context(context_), timer(), log(&Logger::get("ClusterManagerService"))
{
    ...
    std::ostringstream os_pid;
    os_pid << proc.id();
    args.push_back(os_pid.str());
