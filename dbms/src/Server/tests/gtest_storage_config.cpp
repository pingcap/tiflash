...
background_read_weight=2
    ...
    auto verifyDefault = [](const StorageIORateLimitConfig& io_config)
    {
        ...
        ASSERT_EQ(io_config.fg_write_weight, 10);
        ASSERT_EQ(io_config.bg_write_weight, 10);
        ASSERT_EQ(io_config.fg_read_weight, 70);
        ASSERT_EQ(io_config.bg_read_weight, 10);
        ASSERT_EQ(io_config.readWeight(), 80);
        ASSERT_EQ(io_config.writeWeight(), 20);
