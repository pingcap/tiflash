    ...
    try
    {
        ...
        *is_shutdown = true;
        ...
        if (GET_METRIC(tiflash_object_count, type_count_of_mpptunnel).Value() >= 1)
            LOG_WARNING(log, "Wait {} seconds for mpp tunnel, still some mpp tunnel is alive, potential resource leak", wait_cnt);
        else
            LOG_WARNING(log, "Wait {} seconds for mpp tunnel, all finished", wait_cnt);
