...
int migrateEntry(const std::vector<std::string> & opts)
{
    ...
    try
    {
        ...
        if (args.version == 2)
        {
            ...
            if (algorithm_ == "xxh3")
            {
                args.algorithm = DB::DM::ChecksumAlgo::XXH3;
            }
            else if (algorithm_ == "crc32")
            {
                args.algorithm = DB::DM::ChecksumAlgo::CRC32;
            }
            else if (algorithm_ == "crc64")
            {
                args.algorithm = DB::DM::ChecksumAlgo::CRC64;
            }
            else if (algorithm_ == "city128")
            {
                args.algorithm = DB::DM::ChecksumAlgo::City128;
            }
            else if (algorithm_ == "none")
            {
                args.algorithm = DB::DM::ChecksumAlgo::None;
            }
            else
            {
