...
constexpr bool is_any_int_vs_uint = is_integer_v<TInt> && is_integer_v<TUInt> &&
                               is_signed_v<TInt> && is_unsigned_v<TUInt>;
...
using bool_if_double_can_be_used = std::enable_if_t<
                                        is_integer_v<TAInt> && (sizeof(TAInt) <= 4) && std::is_floating_point_v<TAFloat>,
                                        bool>;
...
{
    return u.items[0] == 0 && greaterOp(f, u.items[1]);
}

template <>
inline bool greaterOp<DB::UInt128, DB::Float64>(DB::UInt128 u, DB::Float64 f)
{
    return u.items[0] != 0 || greaterOp(u.items[1], f);
}
