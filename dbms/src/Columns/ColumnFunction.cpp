...
ColumnPtr ColumnFunction::permute(const Permutation & perm, size_t limit) const
{
    ...
    if (perm.size() < limit)
        throw Exception(
            "Size of permutation (" + toString(perm.size())
...
void ColumnFunction::appendArguments(const ColumnsWithTypeAndName & columns)
{
    ...
    if (were_captured + wanna_capture > args)
        throw Exception(
            "Cannot capture " + toString(wanna_capture) + " columns because function " + function->getName()
...
ColumnWithTypeAndName ColumnFunction::reduce() const
{
    ...
    if (args != captured)
        throw Exception(
            "Cannot call function " + function->getName() + " because is has " + toString(args)
                + "arguments but " + toString(captured) + " columns were captured.",
            ErrorCodes::LOGICAL_ERROR);
