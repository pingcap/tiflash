...
ColumnPtr ColumnConst::replicate(size_t /*start_row*/, size_t end_row, size_t prev_offset, const IColumn::Offsets & offsets) const
{
    if (s != offsets.size())
        throw Exception(
            fmt::format("Size of offsets ({}) doesn't match size of column ({})", offsets.size(), s),
    ...
    size_t replicated_size = 0 == s ? 0 : (offsets[end_row] - prev_offset);
    return ColumnConst::create(data, replicated_size);
}
