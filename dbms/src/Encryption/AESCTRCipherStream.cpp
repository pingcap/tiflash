...
void AESCTRCipherStream::cipher(uint64_t file_offset, char* data,
  ...
  if (block_offset > 0) {
    ...
    if (ret != 1)
    {
        throw Exception("Crypter failed for first block, offset " +
                             std::to_string(file_offset), ErrorCodes::DATA_ENCRYPTION_ERROR);
    }
    if (output_size != AES_BLOCK_SIZE)
    {
        throw Exception("Unexpected crypter output size for first block, expected " +
          std::to_string(AES_BLOCK_SIZE) + " vs actual " + std::to_string(output_size), ErrorCodes::DATA_ENCRYPTION_ERROR);
    }
  ...
  if (remaining_data_size >= AES_BLOCK_SIZE) {
    ...
    if (ret != 1) {
        throw Exception("Crypter failed at offset " +
                             std::to_string(file_offset + data_offset), ErrorCodes::DATA_ENCRYPTION_ERROR);
    }
    if (output_size != static_cast<int>(actual_data_size)) {
        throw Exception("Unexpected crypter output size, expected " +
                             std::to_string(actual_data_size) + " vs actual " +
                             std::to_string(output_size), ErrorCodes::DATA_ENCRYPTION_ERROR);
    }
  ...
  if (remaining_data_size > 0)
  {
    ...
    ret = EVP_CipherUpdate(ctx, partial_block, &output_size, partial_block,
                           AES_BLOCK_SIZE);
    if (ret != 1) {
        throw Exception("Crypter failed for last block, offset " +
                             std::to_string(file_offset + data_offset), ErrorCodes::DATA_ENCRYPTION_ERROR);
    }
    if (output_size != AES_BLOCK_SIZE) {
        throw Exception("Unexpected crypter output size for last block, expected " +
          std::to_string(AES_BLOCK_SIZE) + " vs actual " + std::to_string(output_size), ErrorCodes::DATA_ENCRYPTION_ERROR);
    }
