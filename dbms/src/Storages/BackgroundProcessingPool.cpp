...
{
    LOG_FMT_INFO(Logger::get(), "Create BackgroundProcessingPool, prefix={} n_threads={}", thread_prefix, size);
...
std::vector<pid_t> BackgroundProcessingPool::getThreadIds()
{
    ...
    {
        LOG_FMT_ERROR(Logger::get(), "thread_ids.size is {}, but {} is required", thread_ids.size(), size);
