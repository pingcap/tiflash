    ...
    std::function<String(const std::vector<T> & paths, size_t idx)> path_generator, Poco::Logger * log, const String & log_msg)
{
    ...
    {
        struct statvfs vfs;
        if (int code = statvfs(paths[i].path.data(), &vfs); code != 0)
        {
            // This disk may being hot remove, Just give up this disk.
            LOG_ERROR(log, "Could not calculate available disk space (statvfs) of path: " << paths[i].path << ", errno: " << errno);
            continue;
        }

        FsStats path_stat = global_capacity->getFsStatsOfPath(paths[i].path);
        ...
        // update fs_path_map
        {
            auto entry = fs_path_map.find(vfs.f_fsid);
    ...
    for (auto fs_it = disk_stats_map.begin(); fs_it != disk_stats_map.end(); ++fs_it)
    {
        ...
        {
            disk_stat.avail_size += (*path_stats_it).avail_size;
        }
        ...
        if (disk_stat.avail_size > biggest_avail_size)
        {
            biggest_avail_size = disk_stat.avail_size;
            biggest_fs = fs_it->first;
        }

        total_available_size += disk_stat.avail_size;
    }
    ...
    FsStats single_path_stat;
    size_t index_in_paths;
    for (size_t i = 0; i < select_paths.size(); ++i)
    {
        std::tie(single_path_stat, index_in_paths) = path_capacity.find(select_paths[i])->second;
