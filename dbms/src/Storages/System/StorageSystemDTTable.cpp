    ...
    const unsigned /*num_streams*/)
{
    ...
    for (const auto & d : databases)
    {
        ...
        for (; it->isValid(); it->next())
        {
            ...
            res_columns[2]->insert(stat.segment_count);

            res_columns[3]->insert(stat.total_rows);
            res_columns[4]->insert(stat.total_size);
            res_columns[5]->insert(stat.total_delete_ranges);

            res_columns[6]->insert(stat.delta_rate_rows);
            res_columns[7]->insert(stat.delta_rate_segments);

            res_columns[8]->insert(stat.delta_placed_rate);
            res_columns[9]->insert(stat.delta_cache_size);
            res_columns[10]->insert(stat.delta_cache_rate);
            res_columns[11]->insert(stat.delta_cache_wasted_rate);

            res_columns[12]->insert(stat.avg_segment_rows);
            res_columns[13]->insert(stat.avg_segment_size);

            res_columns[14]->insert(stat.delta_count);
            res_columns[15]->insert(stat.total_delta_rows);
            res_columns[16]->insert(stat.total_delta_size);
            res_columns[17]->insert(stat.avg_delta_rows);
            res_columns[18]->insert(stat.avg_delta_size);
            res_columns[19]->insert(stat.avg_delta_delete_ranges);

            res_columns[20]->insert(stat.stable_count);
            res_columns[21]->insert(stat.total_stable_rows);
            res_columns[22]->insert(stat.total_stable_size);
            res_columns[23]->insert(stat.avg_stable_rows);
            res_columns[24]->insert(stat.avg_stable_size);

            res_columns[25]->insert(stat.total_pack_count_in_delta);
            res_columns[26]->insert(stat.avg_pack_count_in_delta);
            res_columns[27]->insert(stat.avg_pack_rows_in_delta);
            res_columns[28]->insert(stat.avg_pack_size_in_delta);

            res_columns[29]->insert(stat.total_pack_count_in_stable);
            res_columns[30]->insert(stat.avg_pack_count_in_stable);
            res_columns[31]->insert(stat.avg_pack_rows_in_stable);
            res_columns[32]->insert(stat.avg_pack_size_in_stable);

            res_columns[33]->insert(stat.storage_stable_num_snapshots);
            res_columns[34]->insert(stat.storage_stable_num_pages);
            res_columns[35]->insert(stat.storage_stable_num_normal_pages);
            res_columns[36]->insert(stat.storage_stable_max_page_id);

            res_columns[37]->insert(stat.storage_delta_num_snapshots);
            res_columns[38]->insert(stat.storage_delta_num_pages);
            res_columns[39]->insert(stat.storage_delta_num_normal_pages);
            res_columns[40]->insert(stat.storage_delta_max_page_id);

            res_columns[41]->insert(stat.storage_meta_num_snapshots);
            res_columns[42]->insert(stat.storage_meta_num_pages);
            res_columns[43]->insert(stat.storage_meta_num_normal_pages);
            res_columns[44]->insert(stat.storage_meta_max_page_id);

            res_columns[45]->insert(stat.background_tasks_length);
        }
    }
