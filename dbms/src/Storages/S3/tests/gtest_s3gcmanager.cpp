...
try
{
    ...
    { // prepare the manifest on S3
        ...
        });
        {
            ...
            edits.varEntry(
                "apple",
                PageVersion(2),
                PageEntryV3{.checkpoint_info = {CheckpointLocation{.data_file_id = std::make_shared<String>("apple_lock")}, false}},
                1);
            edits.varExternal(
                "banana",
                PageVersion(3),
                PageEntryV3{.checkpoint_info = {CheckpointLocation{.data_file_id = std::make_shared<String>("banana_lock")}, true}},
                1);
    ...
    EXPECT_EQ(locks.size(), 2) << fmt::format("{}", locks);
