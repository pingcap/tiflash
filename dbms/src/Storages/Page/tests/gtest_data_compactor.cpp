...
try
{
    ...
    {
        // Apply migration
        auto [edits, bytes_written] = compactor.migratePages(snapshot, valid_pages, candidates, PageFileSet{}, PageFileSet{}, 0);
    ...
    for (size_t i = 0; i < delegate->numPaths(); ++i)
    {
        ...
        std::tie(std::ignore, bytes_written) = compactor.migratePages(snapshot, valid_pages, candidates, PageFileSet{}, PageFileSet{}, 0);
