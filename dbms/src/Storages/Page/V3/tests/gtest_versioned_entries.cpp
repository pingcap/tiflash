...
try
{
    ...
    // insert some entries
    entries.createNewExternal(PageVersion(2), std::nullopt);
    ...
    // insert entry after delete
    entries.createNewExternal(PageVersion(7), std::nullopt);
...
TEST_F(VersionedEntriesTest, getEntriesByBlobId)
{
    ...
    // {1, 2, 3, 100}; blob_id 2,3,100 is not exist in actual
    {
        u128::PageDirectoryTrait::GcEntriesMap blob_entries;
        std::map<PageIdV3Internal, std::tuple<PageIdV3Internal, PageVersion>> rewrite;
