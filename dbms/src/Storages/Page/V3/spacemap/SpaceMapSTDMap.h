...
protected:
    ...
    bool markUsedImpl(UInt64 offset, size_t length) override
    {
        ...
        {
            LOG_FMT_WARNING(log, "Marked space used failed. [offset={}, size={}] is bigger than space [offset={},size={}]", 
                            offset, length, it->first, it->second);
            return false;
        }
    ...
    std::pair<UInt64, UInt64> searchInsertOffset(size_t size) override
    {
        ...
        if (it == free_map.end())
        {
            LOG_FMT_ERROR(log, "Not sure why can't found any place to insert. [size={}] [old biggest_offset={}] [old biggest_cap={}] [new biggest_offset={}] [new biggest_cap={}]", 
                          size, hint_biggest_offset, hint_biggest_cap, scan_biggest_offset, scan_biggest_cap);
    ...
    bool markFreeImpl(UInt64 offset, size_t length) override
    {
        ...
        if (it != free_map.begin())
        {
            ...
            {
                LOG_FMT_WARNING(log, "Marked space free failed. [offset={}, size={}], prev node is [offset={},size={}]", 
                                it->first, it->second, it_prev->first, it_prev->second);
                free_map.erase(it);
                return false;
            }
        }
        ...
        if (it_next != free_map.end())
        {
            ...
            {
                LOG_FMT_WARNING(log, "Marked space free failed. [offset={}, size={}], next node is [offset={},size={}]", 
                                it->first, it->second, it_next->first, it_next->second);
                free_map.erase(it);
                return false;
            }
        }
