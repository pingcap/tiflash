...
public:
    ...
    {
        uwb.putPage(UniversalPageIdFormat::toFullPageId(prefix, page_id), tag, read_buffer, size, data_sizes);
    }

    void putExternal(PageIdU64 page_id, UInt64 tag)
    {
        uwb.putExternal(UniversalPageIdFormat::toFullPageId(prefix, page_id), tag);
    }

    // Add RefPage{ref_id} -> Page{page_id}
    void putRefPage(PageIdU64 ref_id, PageIdU64 page_id)
    {
        uwb.putRefPage(UniversalPageIdFormat::toFullPageId(prefix, ref_id), UniversalPageIdFormat::toFullPageId(prefix, page_id));
    }

    void delPage(PageIdU64 page_id)
    {
        uwb.delPage(UniversalPageIdFormat::toFullPageId(prefix, page_id));
    }
