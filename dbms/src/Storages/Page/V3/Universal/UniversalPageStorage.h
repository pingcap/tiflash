...
public:
    ...
    struct DumpCheckpointOptions
    {
         ...
         * checkpoint
         */
        double remote_gc_threshold = 0.2;
    };

    void dumpIncrementalCheckpoint(const DumpCheckpointOptions & options);

    std::unordered_set<String> getRemoteFileIdsNeedCompact(double gc_threshold);
    ...
    {
        bool not_skip = false;
        WriteLimiterPtr write_limiter = nullptr;
        ReadLimiterPtr read_limiter = nullptr;
    };
    bool gc(const GCOptions & opts);
