...
static void rb_print_stats(struct bitmaps * bitmap)
{
    ...
    if (min_size == ULONG_MAX)
        min_size = 0;
    eff = (double)((count * sizeof(struct bmap_rb_extent)) << 3) / (bitmap->real_end - bitmap->start);

    printf("%16llu extents (%llu bytes)\n", count, ((count * sizeof(struct bmap_rb_extent)) + sizeof(struct rb_private)));
    printf("%16llu bits minimum size\n", min_size);
    printf("%16llu bits maximum size\n"
           "%16llu bits average size\n",
           max_size,
           avg_size);
    printf("%16llu bits set in bitmap (out of %llu)\n", size, bitmap->real_end - bitmap->start);
...
int unmark_block_bmap_range(struct bitmaps * bitmap, UInt64 block, unsigned int num)
{
    ...
    if ((block < bitmap->start) || (block > bitmap->end) || (block + num - 1 > bitmap->end))
    {
        return -1;
    }
...
{
    if ((start < bitmap->start) || (start > bitmap->end) || (start + num - 1 > bitmap->end))
    {
        return -1;
    }
