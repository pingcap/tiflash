...
// TODO REVIEW: need a doc of dir structure
...
PageMap PageStorage::read(std::vector<PageId> page_ids)
{
...
void PageStorage::traverse(std::function<void(const Page & page)> acceptor)
{
    std::map<PageFileIdAndLevel, PageIds> file_and_pages;
    {
        ...
        // TODO REVIEW: memory usage or fd usage maybe too much
...
bool PageStorage::gc()
{
    ...
    for (auto & page_file : page_files)
    {
        ...
        bool is_candidate = page_file.fileIdLevel() != write_file.fileIdLevel()
...

        {
            ...
            for (const auto & file_id_level : merge_files)
            {
                ...
                if (!page_id_and_caches.empty())
                {
                    ...
                    WriteBatch wb;
                    // TODO REVIEW: extra memory copy here
    ...
    // TODO delay remove, some read threads may using this files.
    for (const auto & [file_id, level] : merge_files)
    {
