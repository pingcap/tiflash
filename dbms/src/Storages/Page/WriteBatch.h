...
public:
    ...
    void putExternal(PageId page_id, UInt64 tag)
    {
        ...
        Write w{WriteBatchWriteType::PUT_EXTERNAL, page_id, tag, nullptr, 0, 0, {}, 0, 0, {}, std::nullopt};
        writes.emplace_back(std::move(w));
    }

    void putRemoteExternal(PageId page_id, UInt64 tag, const PS::RemoteDataLocation & remote_location)
    {
        // External page's data is not managed by PageStorage, which means data is empty.
