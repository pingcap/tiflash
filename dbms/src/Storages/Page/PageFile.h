...
public:
    ...
    /// Get the page files which covered by this one.
    /// Only page files with level >= 1 contain the information. They are generated by GC.
    std::unique_ptr<PageFileIdAndLevels> readCoveredPageFiles();
    /// Write the page files which covered by this one.
    void writeCoveredPageFiles(const PageFileIdAndLevels & covered);
...
private:
    ...
    std::string capsPath() const { return folderPath() + "/caps"; } // current available pages.
    std::string cpfsPath() const { return folderPath() + "/cpfs"; } // covered page files.
