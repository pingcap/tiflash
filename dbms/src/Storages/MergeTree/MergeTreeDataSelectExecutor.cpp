        ...
        { // learner read
            ...
            const auto func_run_learner_read = [&](const size_t region_begin) {
                ...
                for (size_t region_index = region_begin; region_index < region_end; ++region_index)
                {
                    ...
                    auto [block, status] = RegionTable::getBlockInputStreamByRegion(data.table_info->id, region, region_query_info.version,
                        region_query_info.conf_version, *data.table_info, data.getColumns(), tmt_column_names_to_read,
