    ...
    Int64 max_block_number_to_read) const
{
    ...
    if (is_txn_engine)
    {
        ...
        else
        {
            ...
            // TODO REVIEW: this may lift up to Executor's caller
        ...
        // TODO REVIEW: log the elapsed time
        pool.wait();
    ...
    else
    {
        ...
        // TODO REVIEW: throw a different exception?
    ...
    // TODO REVIEW: more events, eg: need_retry, regions
    ...
    // TODO REVIEW: may split to some small methods
    if (is_txn_engine)
    {
        ...
        else
        {
            for (size_t region_begin = 0, size = std::max(region_cnt / num_streams, 1); region_begin < region_cnt; region_begin += size)
            {
                ...
                for (size_t region_index = region_begin, region_end = std::min(region_begin + size, region_cnt); region_index < region_end; ++region_index)
                {
                    ...
                    // TODO REVIEW: optimize `merging == 1`
                    if (!merging.empty())
