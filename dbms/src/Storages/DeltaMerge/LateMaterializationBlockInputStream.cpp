...
Block LateMaterializationBlockInputStream::readImpl()
{
    ...
    while (true)
    {
        ...
        if (std::find(filter->cbegin(), filter->cend(), 1) == filter->cend())
        {
        ...
        else
        {
            ...
            RUNTIME_CHECK_MSG(rest_column_block, "Late materialization meets unexpected end of stream");
            RUNTIME_CHECK_MSG(rest_column_block.rows() == filter_column_block.rows() && rest_column_block.startOffset() == filter_column_block.startOffset(),
                              "Late materialization meets unexpected size of block unmatched, filter_column_block: [start_offset={}, rows={}], rest_column_block: [start_offset={}, rows={}]",
                              filter_column_block.startOffset(),
                              filter_column_block.rows(),
                              rest_column_block.startOffset(),
                              rest_column_block.rows());

            size_t rows = rest_column_block.rows();

            // Get mvcc-filter
            mvcc_filter.resize(rows);
            bool all_match = bitmap_filter->get(mvcc_filter, rest_column_block.startOffset(), rows);
            if (!all_match)
            {
            ...
            if (passed_count == 0)
            {
                // if all rows are filtered, read next block
                continue;
            }
            else if (passed_count != rows)
            {
