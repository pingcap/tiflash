                                           ...
                                           size_t expected_block_size)
{
    ...
    for (;;)
    {
        ...
        if (likely(blk))
        {
            bitmap_filter->set(blk.segmentRowIdCol(), blk.startOffset());
        }
        else
        {
            break;
...
BlockInputStreamPtr Segment::getLateMaterializationStream(BitmapFilterPtr && bitmap_filter,
                                                          ...
                                                          size_t expected_block_size)
{
    assert(filter && filter->beofre_where);
    auto enable_handle_clean_read = !hasColumn(columns_to_read, EXTRA_HANDLE_COLUMN_ID);
    constexpr auto is_fast_scan = true;
    auto enable_del_clean_read = !hasColumn(columns_to_read, TAG_COLUMN_ID);
    ...
    BlockInputStreamPtr filter_column_stream = std::make_shared<ColumnOrderedSkippableBlockInputStream>(
        filter_columns,
    ...
    filter_column_stream->setExtraInfo("projection after push down filter");
    ...
    SkippableBlockInputStreamPtr rest_column_stream = std::make_shared<ColumnOrderedSkippableBlockInputStream>(
        rest_columns_to_read,
                                                        ...
                                                        size_t expected_block_size)
{
    ...
    {
        // if enable late materialization, and has filter conditions pushed down, use late materialization
