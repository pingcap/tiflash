    ...
    size_t expected_block_size)
{
    ...
    if (unlikely(filter_columns->size() == columns_to_read.size()))
    {
        ...
        BlockInputStreamPtr stream = std::make_shared<BitmapFilterBlockInputStream>(
            *filter_columns,
            ...
            bitmap_filter,
            dm_context.tracing_id);
        stream = getFilterInputStream(stream, filter->lm_filter, /*need_project*/ true, dm_context.tracing_id);
        return stream;
    }
    ...
    filter_column_stream
        = getFilterInputStream(filter_column_stream, filter->lm_filter, /*need_project*/ false, dm_context.tracing_id);
