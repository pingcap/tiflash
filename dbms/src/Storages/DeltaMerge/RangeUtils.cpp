...
{
private:
    HandleRanges sorted_ranges;
    OffsetCounts merged_stats;

public:
    MergeRangeHelper(HandleRanges && sorted_ranges_) : sorted_ranges(std::move(sorted_ranges_)) //
    {
        genMergeStats();
    }
    ...
    HandleRanges getRanges()
    {
        DM::HandleRanges merged_ranges;
        merged_ranges.reserve(merged_stats.size());
        for (auto & stat : merged_stats)
        {
            DM::HandleRange range(sorted_ranges[stat.offset].start, sorted_ranges[stat.offset + stat.count - 1].end);
...
{
    std::sort(ranges.begin(), ranges.end(), [](const HandleRange & lhs, const HandleRange & rhs) {
