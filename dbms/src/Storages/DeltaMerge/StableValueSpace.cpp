...
void StableValueSpace::calculateDMFileProperty(const DMContext & context, const RowKeyRange & rowkey_range, bool is_common_handle)
{
    ...
    {
        auto & file = files[i];
        // TODO: we don't need all the pack_stats, after logical split, just a part of it belong to this segment
        auto & pack_stats = file->getPackStats();
        auto & pack_propertys = file->getPackPropertys();
        DMFile::PackPropertys new_pack_propertys;
        ...
        if (pack_propertys.property_size() == 0)
        {
        ...
        size_t new_pack_propertys_index = 0;
        bool use_new_pack_propertys = pack_propertys.property_size() == 0;
            ...
            if (use_new_pack_propertys)
            {
                if (unlikely((size_t)new_pack_propertys.property_size() <= new_pack_propertys_index))
                {
                    throw Exception("new_pack_propertys size " + toString(new_pack_propertys.property_size())
                        + " doesn't contain all info for packs", ErrorCodes::LOGICAL_ERROR);
                }
                property.num_rows += new_pack_propertys.property(new_pack_propertys_index).num_rows();
                new_pack_propertys_index += 1;
            }
            else
            {
                property.num_rows += pack_propertys.property(pack_id).num_rows();
            }
        }
    }
