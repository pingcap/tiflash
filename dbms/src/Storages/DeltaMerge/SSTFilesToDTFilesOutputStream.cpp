...
void SSTFilesToDTFilesOutputStream::writeSuffix()
{
    ...
    LOG_FMT_INFO(
        log,
        "Transformed snapshot in SSTFile to DTFiles, region={} cost_ms={} rows={} bytes={} write_cf_keys={} default_cf_keys={} lock_cf_keys={} dt_files=[{}]",
...
std::vector<ExternalDTFileInfo> SSTFilesToDTFilesOutputStream::outputFiles() const
{
    RUNTIME_CHECK(
        ingest_files.size() == ingest_files_range.size(),
        Exception,
        fmt::format("Check ingest_files.size() == ingest_files_range.size() failed, ingest_files.size={} ingest_files_range.size={}",
                    ingest_files.size(),
    ...
    for (size_t i = 0; i < ingest_files.size(); i++)
    {
        ...
        RUNTIME_CHECK(ingest_files_range[i].has_value(), Exception, fmt::format("Check ingest_files_range[i].has_value failed"));
...
void SSTFilesToDTFilesOutputStream::updateRangeFromNonEmptyBlock(Block & block)
{
    ...
    RUNTIME_CHECK(compare(block_start, block_end.toRowKeyValueRef()) < 0, Exception, "Check compare(block_start, block_end.toRowKeyValueRef()) < 0 failed");
    ...
    {
        RUNTIME_CHECK(compare(block_start, current_file_range->getStart()) > 0, Exception, "Check compare(block_start, current_file_range->getStart()) > 0 failed");
        current_file_range->setEnd(block_end);
    }
    RUNTIME_CHECK(!current_file_range->none(), Exception, "Check !current_file_range->none() failed");
}
