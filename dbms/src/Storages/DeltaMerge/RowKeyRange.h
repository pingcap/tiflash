...
struct RowKeyValue
{
    ...
    RowKeyValue(bool is_common_handle_, const HandleValuePtr value_)
        : is_common_handle(is_common_handle_)
        , value(value_)
    {
        if (is_common_handle)
        ...
        if (unlikely(value->size() > sizeof(UInt64)))
        {
            ...
            if (int_value < std::numeric_limits<Int64>::max())
            {
                LOG_WARNING(
                    Logger::get(),
                    "Meet rowkey which has extra suffix, rowkey={} suffix={}",
                    Redact::keyToDebugString(value->data(), value->size()),
                    Redact::keyToDebugString(value->data() + sizeof(UInt64), value->size() - sizeof(UInt64)));
