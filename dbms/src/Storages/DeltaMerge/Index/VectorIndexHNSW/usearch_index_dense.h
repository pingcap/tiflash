...
/**
 * @brief This is a modified version of usearch's index_dense.hpp.
 * It supports predicate fn when doing the vector search.
 *
 * Original implementation: https://github.com/unum-cloud/usearch/blob/v2.9.1/include/usearch/index_dense.hpp
 */

// NOLINTBEGIN(readability-*,google-*,modernize-use-auto)
...
struct index_dense_config_t : public index_config_t
{
    ...
    index_dense_config_t(index_config_t base) noexcept
        : index_config_t(base)
    {}

    index_dense_config_t(
        std::size_t c = default_connectivity(),
        std::size_t ea = default_expansion_add(),
        std::size_t es = default_expansion_search()) noexcept
        : index_config_t(c)
        , expansion_add(ea ? ea : default_expansion_add())
...
struct index_dense_copy_config_t : public index_copy_config_t
{
    ...
    index_dense_copy_config_t(index_copy_config_t base) noexcept
        : index_copy_config_t(base)
    {}
};
...
struct index_dense_metadata_result_t
{
    ...
    index_dense_metadata_result_t() noexcept
        : config()
        , head_buffer()
        , head(head_buffer)
        , error()
...
private:
    ...
    public:
        metric_proxy_t(index_dense_gt const & index) noexcept
            : index_(&index)
        {}
...
public:
    ...
    search_result_t search(b1x8_t const* vector, std::size_t wanted, predicate_at&& predicate = predicate_at{}, std::size_t thread = any_thread(), size_t expansion = default_expansion_search(), bool exact = false) const { return search_(vector, wanted, predicate, thread, expansion, exact, casts_.from_b1x8); }
    template <typename predicate_at = dummy_predicate_t>
    search_result_t search(i8_t const* vector, std::size_t wanted, predicate_at&& predicate = predicate_at{}, std::size_t thread = any_thread(), size_t expansion = default_expansion_search(), bool exact = false) const { return search_(vector, wanted, predicate, thread, expansion, exact, casts_.from_i8); }
    template <typename predicate_at = dummy_predicate_t>
    search_result_t search(f16_t const* vector, std::size_t wanted, predicate_at&& predicate = predicate_at{}, std::size_t thread = any_thread(), size_t expansion = default_expansion_search(), bool exact = false) const { return search_(vector, wanted, predicate, thread, expansion, exact, casts_.from_f16); }
    template <typename predicate_at = dummy_predicate_t>
    search_result_t search(f32_t const* vector, std::size_t wanted, predicate_at&& predicate = predicate_at{}, std::size_t thread = any_thread(), size_t expansion = default_expansion_search(), bool exact = false) const { return search_(vector, wanted, predicate, thread, expansion, exact, casts_.from_f32); }
    template <typename predicate_at = dummy_predicate_t>
    search_result_t search(f64_t const* vector, std::size_t wanted, predicate_at&& predicate = predicate_at{}, std::size_t thread = any_thread(), size_t expansion = default_expansion_search(), bool exact = false) const { return search_(vector, wanted, predicate, thread, expansion, exact, casts_.from_f64); }
    ...
    cluster_result_t cluster(vector_key_t key, std::size_t level, std::size_t thread = any_thread()) const
    {
        ...
        auto allow = [=, this](member_cref_t const & member) noexcept {
    ...
    serialization_result_t save_to_stream(
        output_callback_at && output, //
    ...
    serialization_result_t load_from_stream(
        input_callback_at && input, //
        serialization_config_t config = {}, //
     ...
     */
    template <typename progress_at = dummy_progress_t>
    serialization_result_t save(output_file_t file, serialization_config_t config = {}, progress_at && progress = {})
        const
    {
    ...
    public:
        values_proxy_t(index_dense_gt const & index) noexcept
            : index_(&index)
        {}
        ...
        progress_at && progress = progress_at{}) const
    {
        ...
        return unum::usearch::join( //
            ...
            *women.typed_, //
            values_proxy_t{men},
            values_proxy_t{women}, //
            metric_proxy_t{men},
            metric_proxy_t{women}, //
            config, //
        ...
        cast_t const & cast) const
    {
        ...
        auto allow = [=, this](member_cref_t const & member) noexcept {
            if (member.key == free_key_)
                return false;
            if constexpr (!is_dummy<predicate_at>())
...
// NOLINTEND(readability-*,google-*,modernize-use-auto)
