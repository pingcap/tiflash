...
public:
    ...
    ColumnRangePtr buildSets(const google::protobuf::RepeatedPtrField<tipb::ColumnarIndexInfo> & index_infos) override
    {
        ...
        {
            set = IntegerSet::createGreaterRangeSet(attr.type, *domain.lower, /*not_included=*/!domain.lower_inclusive);
        }
        else if (domain.upper)
        {
            set = IntegerSet::createLessRangeSet(attr.type, *domain.upper, /*not_included=*/!domain.upper_inclusive);
        }
