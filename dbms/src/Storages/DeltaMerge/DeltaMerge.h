    ...
    void beforeReturnBlock(const Block & block)
    {
        if constexpr (DM_RUN_CHECK)
        {
            ...
            for (size_t i = 0; i < handle_column.size(); ++i)
            {
                ...
                {
                    throw Exception("DeltaMerge return wrong result, current handle[" + DB::Redact::handleToDebugString(handle)
                                    + "]version[" + DB::toString(version) + "]@read[" + DB::toString(num_read) + "]@pos[" + DB::toString(i)
                                    + "] is expected >= last_handle[" + DB::Redact::handleToDebugString(last_handle) + "]last_version["
