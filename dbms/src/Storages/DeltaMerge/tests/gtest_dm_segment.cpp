...
protected:
    SegmentPtr reload(const ColumnDefinesPtr & pre_define_columns = {}, DB::Settings && db_settings = DB::Settings())
    {
        ...
        return Segment::newSegment(*dm_context, table_columns, RowKeyRange::newAll(false, 1), storage_pool->newMetaPageId(), 0, /* log_prefix */ "");
    }
...
try
{
    ...
    SegmentPtr new_segment = Segment::restoreSegment(dmContext(), segment->segmentId(), /* log_prefix */ "");
...

    {
        ...
        segment->write(dmContext(), {RowKeyRange::fromHandleRange(del)});
        new_segment = segment->restoreSegment(dmContext(), segment->segmentId(), /* log_prefix */ "");
    }
...
try
{
    ...
    {
        SegmentPtr new_segment_1 = Segment::restoreSegment(dmContext(), segment->segmentId(), /* log_prefix */ "");
        SegmentPtr new_segment_2 = Segment::restoreSegment(dmContext(), other_segment->segmentId(), /* log_prefix */ "");
