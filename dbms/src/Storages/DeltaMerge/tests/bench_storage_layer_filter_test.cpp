...
BENCHMARK_DEFINE_F(DeltaMergeStoreTestForBench, ReadWithFilterOnce)
(benchmark::State & state)
{
    ...
    for (auto _ : state)
    {
        BlockInputStreamPtr in = store->read(*db_context,
                                            ...
                                            {RowKeyRange::fromHandleRange(HandleRange(block_rows / 2, block_rows * file_num),store->isCommonHandle())},
                                             /* num_streams= */ 1,
