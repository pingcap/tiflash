...
try
{
...

    {
        ...
        auto stream = builder
                          ...
                          .build(nullptr, dm_file, *cols, RowKeyRanges{RowKeyRange::newAll(false, 1)});
...
try
{
    ...
    auto test_read_range = [&](const HandleRange & range) {
        ...
        auto stream = builder
                          .setColumnCache(column_cache)
                          .build(nullptr, dm_file, *cols, RowKeyRanges{RowKeyRange::fromHandleRange(range)}); // Filtered by read_range
...
try
{
...

    {
        ...
        auto stream = builder
                          .setColumnCache(column_cache)
                          .build(nullptr, dm_file, *cols, RowKeyRanges{RowKeyRange::newAll(is_common_handle, rowkey_column_size)});
        // mock common handle
...
try
{
    ...
    for (const auto & range : ranges)
    {
        ...
        auto stream = builder
                          .setColumnCache(column_cache)
                          .build(nullptr, dm_file, *cols, RowKeyRanges{range.range}); // Filtered by read_range
        Int64 expect_first_pk = static_cast<int>(std::floor(std::max(0, range.start) / span_per_part)) * span_per_part;
        Int64 expect_last_pk = std::min(num_rows_write, //
                                        static_cast<int>(std::ceil(std::min(num_rows_write, range.end) / span_per_part)) * span_per_part
                                            + (range.end % span_per_part ? span_per_part : 0));
...
try
{
    ...
    *cols_after_ddl = cols_before_ddl;
...

    {
        ...
        auto stream = builder
                          .setColumnCache(column_cache)
                          .build(nullptr, dm_file, *cols_after_ddl, RowKeyRanges{RowKeyRange::newAll(false, 1)});
        ASSERT_INPUTSTREAM_COLS_UR(
            stream,
