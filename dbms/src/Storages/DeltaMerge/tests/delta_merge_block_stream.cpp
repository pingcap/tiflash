...
void deltaTreeInsertTest(Context context)
{
    ...
    ValueSpacePtr insert_value_space = std::make_shared<MemoryValueSpace>("insert_value_space", names_and_types_list,
                                                                          SortDescription{});
    ValueSpacePtr modify_value_space = std::make_shared<MemoryValueSpace>("modify_value_space", names_and_types_list,
                                                                          SortDescription{});
...
void deltaTreeOutputInsertTest(Context context)
{
    ...
    BlockOutputStreamPtr delta_output_stream = std::make_shared<DeltaMergeBlockOutputStream>(input_stream_creator, delta_tree,
                                                                                      Action::Insert, sd, []() {}, 10000);
    ...
    while (Block block = dms.read())
    {
        for (auto iter = block.begin(); iter != block.end(); iter++)
        {
            ...
            for (unsigned int i = 0; i < c->size(); i++)
            {
                ...
                {
                    //std::cout << c->getUInt(i) << std::endl;
                    myAssert(c->getUInt(i) == 2*i);
                } else if (iter->name == "col2")
                {
                    //std::cout << c->getDataAt(i) << std::endl;
                    myAssert(c->getDataAt(i) == "b");
                }
            }
        }
    }
