...
void DeltaMergeStore::write(const Context & db_context, const DB::Settings & db_settings, Block && to_write)
{
    ...
    while (offset != rows)
    {
        ...
        WriteBatches   wbs(storage_pool, db_context.getIORateLimiter().getWriteLimiter());
                                  ...
                                  bool                        clear_data_in_range)
{
    ...
    WriteBatches ingest_wbs(storage_pool, dm_context->db_context.getIORateLimiter().getWriteLimiter());
    if (files.size() > 0)
    {
    ...
    while (!cur_range.none())
    {
        ...
        while (true)
        {
            ...
            // Write could fail, because other threads could already updated the instance. Like split/merge, merge delta.
            DeltaPacks   packs;
            WriteBatches wbs(storage_pool, dm_context->db_context.getIORateLimiter().getWriteLimiter());
