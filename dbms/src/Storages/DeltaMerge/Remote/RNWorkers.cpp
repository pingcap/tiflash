...
RNWorkers::RNWorkers(const Context & context, const Options & options, size_t num_streams)
{
    ...
    {
        fetch_pages_concurrency = std::min(num_streams * settings.dt_fetch_page_concurrency_scale, fetch_pages_concurrency);
    }
    if (settings.dt_prepare_stream_concurrency_scale > 0.0)
    {
        prepare_streams_concurrency = std::min(num_streams * settings.dt_prepare_stream_concurrency_scale, prepare_streams_concurrency);
    }
