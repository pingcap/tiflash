...
private:
    ...
    void readSegments()
    {
        ...
        while (merged_task == nullptr || merged_task->tasks.empty())
        {
            merged_task = SegmentReadTaskScheduler::instance().getMergedTask();
            if (merged_task == nullptr || merged_task->tasks.empty())
            {
