...
StorageDeltaMergeDummy::StorageDeltaMergeDummy(const std::string & path_,
    ...
    : IManageableStorage{columns_},
      ...
      log(&Logger::get("StorageDeltaMergeDummy"))
{
    primary_sort_descr.reserve(primary_expr_ast_->children.size());
    for (const ASTPtr & ast : primary_expr_ast_->children)
    {
        primary_sort_descr.emplace_back(ast->getColumnName(), 1, 1);
    }

    initDelta();
}

void StorageDeltaMergeDummy::initDelta()
{
    ...
    const size_t max_read_buffer_size)
{
    ...
    auto delta_merge_stream_ptr = std::make_shared<DummyDeltaMergeBlockInputStream>(stable_input_stream_ptr, delta_tree, max_block_size);
    return {delta_merge_stream_ptr};
}

BlockOutputStreamPtr StorageDeltaMergeDummy::write(const ASTPtr & query, const Settings & settings)
{
    ...
    DummyDeltaMergeBlockOutputStream::InputStreamCreator input_stream_creator = [this, primary_keys, max_block_size]() {
    ...
    DummyDeltaMergeBlockOutputStream::Flusher flusher = [this]() { this->flushDelta(); };

    return std::make_shared<DummyDeltaMergeBlockOutputStream>(
        input_stream_creator, delta_tree, action, primary_sort_descr, flusher, settings.delta_merge_size);
}

void StorageDeltaMergeDummy::flushDelta()
{
...
UInt64 StorageDeltaMergeDummy::stable_size()
{
    auto stage = QueryProcessingStage::Enum::FetchColumns;
...
std::tuple<UInt64, UInt64, UInt64, UInt64> StorageDeltaMergeDummy::delta_status()
{
    UInt64 entries = 0;
    UInt64 inserts = 0;
    UInt64 deletes = 0;
...
BlockInputStreamPtr StorageDeltaMergeDummy::status()
{
    Block block;
...
void registerStorageDeltaMergeDummy(StorageFactory & factory)
{
    factory.registerStorage("DeltaMerge", [](const StorageFactory::Arguments & args) {
        ...
        return StorageDeltaMergeDummy::create(args.data_path,
            args.table_name,
            args.columns,
            primary_expr_list,
            args.attach,
