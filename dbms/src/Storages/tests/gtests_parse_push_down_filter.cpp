...
DM::PushDownFilterPtr ParsePushDownFilterTest::generatePushDownFilter(const String table_info_json, const String & query, TimezoneInfo & timezone_info)
{
    ...
    {
        std::unordered_set<DM::ColId> filter_column_ids;
        for (const auto & filter : dag_query->pushed_down_filters)
        {
            DM::FilterParser::parseFilterColumnsFromDAGQuery(filter, columns_to_read, filter_column_ids);
        }
...
try
{
...

    {
        ...
        Block before_where_block = Block{{toVec<UInt64>("col_timestamp", {1849559496301477888, 1849559496301477889, 1849559496301977881, 1849259496301477888, 1849559493301477888, 1849559416301477888, 1849559496301477833, 888439}),
    ...
    {
        // origin_time_stamp: 1849560046057291776
