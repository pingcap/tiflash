...
std::shared_ptr<LargeTxnDefaultCf::Inner> & LargeTxnDefaultCf::mustGet(LargeTxnDefaultCf & cf, const LargeTxnDefaultCf::Level1Key & key) {
...
size_t LargeTxnDefaultCf::splitInto(const RegionRange & range, LargeTxnDefaultCf & new_region_data) {
    ...
    for (auto it = txns.begin(); it != txns.end(); it++) {
        size_changed += LargeTxnDefaultCf::mustGet(new_region_data, it->first)->splitInto(range, *new_region_data.txns[it->first]);
    }
    return size_changed;
}
