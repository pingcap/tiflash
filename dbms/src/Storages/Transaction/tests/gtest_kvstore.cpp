...
TEST_F(RegionKVStoreTest, KVStoreFailRecovery)
{
...

    {
        ...
        auto region_id = 2;
        {
            ...
            applied_index = r1->getLatestAppliedIndex();
            ASSERT_EQ(r1->getLatestAppliedIndex(), kvr1->appliedIndex());
            auto index = proxy_instance->normalWrite(kvs, ctx.getTMTContext(), cond, region_id, {34}, {"v1"}, {WriteCmdType::Put}, {ColumnFamilyType::Default});
            proxy_instance->doApply(kvs, ctx.getTMTContext(), cond, region_id, index);
...

    {
        ...
        auto region_id = 3;
        {
            ...
            auto index = proxy_instance->normalWrite(kvs, ctx.getTMTContext(), cond, region_id, {35}, {"v1"}, {WriteCmdType::Put}, {ColumnFamilyType::Default});
            proxy_instance->doApply(kvs, ctx.getTMTContext(), cond, region_id, index);
            ASSERT_EQ(r1->getLatestAppliedIndex(), applied_index);
            ASSERT_EQ(kvr1->appliedIndex(), applied_index + 1);
            kvs.tryPersist(region_id);
        }
        {
            ...
            ASSERT_EQ(r1->getLatestAppliedIndex(), applied_index + 1);
            auto index = proxy_instance->normalWrite(kvs, ctx.getTMTContext(), cond, region_id, {36}, {"v2"}, {WriteCmdType::Put}, {ColumnFamilyType::Default});
            proxy_instance->doApply(kvs, ctx.getTMTContext(), cond, region_id, index);
