...
{
public:
    void checkJsonPathKeyCacheDisable(std::shared_ptr<JsonPathExpr> path_expr)
    {
        for (auto & leg_ptr : path_expr->getLegs())
        {
    ...
    void checkJsonPathKeyUncached(std::shared_ptr<JsonPathExpr> path_expr)
    {
        for (auto & leg_ptr : path_expr->getLegs())
        {
            if (leg_ptr->type == JsonPathLeg::JsonPathLegKey)
...
try
{
    ...
    std::vector<TestData> test_data_vec{
        {json_binary_1, {"$.a"}, true, "[1, \"2\", {\"aa\": \"bb\"}, 4, {\"aa\": \"cc\"}]"},
        {json_binary_2, {"$.a"}, false, ""},
        {json_binary_1, {"$[0]"}, true, "{\"\\\"hello\\\"\": \"world\", \"a\": [1, \"2\", {\"aa\": \"bb\"}, 4, {\"aa\": \"cc\"}], \"b\": true, \"c\": [\"d\"]}"},
        {json_binary_2, {"$[0]"}, true, "{\"a\": 1, \"b\": true}"},
        {json_binary_1, {"$.a[2].aa"}, true, "bb"},
        {json_binary_1, {"$.a[*].aa"}, true, "[\"bb\", \"cc\"]"},
        {json_binary_1, {"$.*[0]"}, true, "[\"world\", 1, true, \"d\"]"},
        {json_binary_1, {"$.a[*].\"aa\""}, true, "[\"bb\", \"cc\"]"},
        {json_binary_1, {"$.\"\\\"hello\\\"\""}, true, "world"},
        {json_binary_1, {"$**[1]"}, true, "[\"2\"]"},
        {json_binary_3, {"$.properties.$type"}, true, "TiDB"},
        {json_binary_4, {"$.properties.$type$type"}, true, "{\"$a$a\": \"TiDB\"}"},
        ...
        {json_binary_1, {"$.a", "$[5]"}, true, "[[1, \"2\", {\"aa\": \"bb\"}, 4, {\"aa\": \"cc\"}]]"},
        {json_binary_2, {"$.a", "$[0]"}, true, "[{\"a\": 1, \"b\": true}]"},
        {json_binary_6, {"$.properties", "$[1]"}, true, "[{\"$type\": {\"$a$a\": \"TiDB\"}}]"},
        {json_binary_6, {"$.hello", "$[2]"}, true, "[{\"$b$b\": \"world\", \"$c\": \"amazing\"}]"},
        {json_binary_7, {"$.a**.b**.c"}, true, "[100]"},
        {json_binary_8, {"$**[0]"}, true, "[{\"a\": {\"b\": [1, 2, 3]}}, {\"b\": [1, 2, 3]}, 1, 2, 3]"},
    ...
    for (size_t i = 0; i < test_data_vec.size(); ++i)
    {
        ...
        for (size_t j = 0; j < test_data_vec[i].path_expr_vec.size(); ++j)
        {
            auto path_expr = JsonPathExpr::parseJsonPathExpr(test_data_vec[i].path_expr_vec[j]);
        ...
        for (size_t j = 0; j < 2; ++j)
        {
            ...
            bool found = test_data_vec[i].bj.extract(path_expr_container_vec, write_buffer);
            ASSERT_TRUE(found == test_data_vec[i].found);
            if (found)
            {
