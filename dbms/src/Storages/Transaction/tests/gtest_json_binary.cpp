...
try
{
    ...
    /// `[1]`
    UInt8 bj10[] = {
        0x1, 0x0, 0x0, 0x0, 0x15, 0x0, 0x0, 0x0, 0x9, 0xd, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0
    };
...
try
{
    ...
    WriteBufferFromVector<ColumnString::Chars_t> write_buffer(data_to);
    {
        ...
        ASSERT_TRUE(found && path_expr->getLegs()[0]->dot_key.status == JsonPathObjectKeyCached &&
...

    {
        ...
        bool found = json_binary_1.extract(path_expr_container_vec, write_buffer);
        ASSERT_TRUE(found);
        ASSERT_TRUE(path_expr->getLegs()[0]->dot_key.status == JsonPathObjectKeyCached &&
                    path_expr->getLegs()[0]->dot_key.cached_index == 1);
        ASSERT_TRUE(path_expr->getLegs()[2]->dot_key.status == JsonPathObjectKeyCached &&
