...
inline DecodingStorageSchemaSnapshotConstPtr getDecodingStorageSchemaSnapshot(const TableInfo & table_info)
{
    ...
    if (handle_id != EXTRA_HANDLE_COLUMN_ID)
    {
        ...
        return std::make_shared<DecodingStorageSchemaSnapshot>(std::make_shared<ColumnDefines>(store_columns), table_info, *iter, /* decoding_schema_epoch_ */ 1);
    }
    else
    {
        return std::make_shared<DecodingStorageSchemaSnapshot>(std::make_shared<ColumnDefines>(store_columns), table_info, store_columns[0], /* decoding_schema_epoch_ */ 1);
    }
}
