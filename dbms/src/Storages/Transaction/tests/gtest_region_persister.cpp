...
void RegionPersisterTest::runTest(const String & path, bool sync_on_write)
{
    Stopwatch watch;
    ...
    LOG_FMT_INFO(&Poco::Logger::get("RegionPersisterTest"), "[sync_on_write={}], [time={:.4f}s]", sync_on_write, seconds);
}

// TODO: check whether we still need this test case
// This test takes about 10 minutes. Disable by default
TEST_F(RegionPersisterTest, DISABLED_persister_sync_on_write)
{
