...
struct CFKeyHasher
{
    inline size_t operator()(const std::pair<HandleID, Timestamp> & k) const noexcept
    {
...
struct RegionWriteCFDataTrait
{
    ...
    static inline std::optional<Map::value_type> genKVPair(TiKVKey && key, const DecodedTiKVKey & raw_key, TiKVValue && value)
    {
...
struct RegionLockCFDataTrait
{
    struct Key
    {
        ...
        struct Hash
        {
            inline size_t operator()(const Key & x) const { return std::hash<std::string_view>()(x.view); }
        };
    ...
    static inline Map::value_type genKVPair(TiKVKey && key_, TiKVValue && value_)
    {
