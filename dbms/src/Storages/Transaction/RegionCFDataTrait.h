...
struct RegionWriteCFDataTrait
{
    ...
    using Value = std::tuple<std::shared_ptr<TiKVKey>, std::shared_ptr<TiKVValue>, DecodedWriteCFValue>;
    ...
    static std::pair<Key, Value> genKVPair(const TiKVKey & key, const String & raw_key, const TiKVValue & value)
    {
        ...
        return {Key{handle_id, ts}, Value{std::make_shared<TiKVKey>(key), std::make_shared<TiKVValue>(value), RecordKVFormat::decodeWriteCfValue(value)}};
    }
...
{
    using Key = std::tuple<HandleID, Timestamp>;
    using Value = std::tuple<std::shared_ptr<TiKVKey>, std::shared_ptr<TiKVValue>>;
    using Map = std::map<Key, Value>;
    ...
    static std::pair<Key, Value> genKVPair(const TiKVKey & key, const String & raw_key, const TiKVValue & value)
    {
        HandleID handle_id = RecordKVFormat::getHandle(raw_key);
        Timestamp ts = RecordKVFormat::getTs(key);
        return {Key{handle_id, ts}, Value{std::make_shared<TiKVKey>(key), std::make_shared<TiKVValue>(value)}};
    }
    static TiKVKey genTiKVKey(const TableID & table_id, const Key & key)
    {
        const auto & [handle_id, ts] = key;
...
struct RegionLockCFDataTrait
{
    ...
    using Key = HandleID;
    using Value = std::tuple<std::shared_ptr<TiKVKey>, std::shared_ptr<TiKVValue>, DecodedLockCFValue>;
    using Map = std::map<Key, Value>;

    static std::pair<Key, Value> genKVPair(const TiKVKey & key, const String & raw_key, const TiKVValue & value)
    {
        HandleID handle_id = RecordKVFormat::getHandle(raw_key);
        return {handle_id, Value{std::make_shared<TiKVKey>(key), std::make_shared<TiKVValue>(value), RecordKVFormat::decodeLockCfValue(value)}};
    }
    static TiKVKey genTiKVKey(const TableID & table_id, const Key & key)
    {
