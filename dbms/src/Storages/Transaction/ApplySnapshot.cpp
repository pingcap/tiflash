...

    {
        ...
        handleRegionsByRangeOverlap(new_range->comparableKeys(), [&](RegionMap region_map, const KVStoreTaskLock &) {
            for (const auto & region : region_map)
            {
                ...
                {
                    LOG_WARNING(log, fmt::format("{}: range of region {} is overlapped with region {}, please check whether its `PeerState` is `Tombstone` in raftstore proxy", __PRETTY_FUNCTION__, region_id, region.first));
                }
            }
        });
    }
