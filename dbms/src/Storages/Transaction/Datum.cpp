...
void unflatten(const Field &, std::optional<Field> &)
{}

template <TP tp, typename = void>
void flatten(const Field &, std::optional<Field> &)
{}

template <TP tp, typename = void>
bool overflow(const Field &, const ColumnInfo &)
{
    return false;
}

template <TP tp, typename std::enable_if<tp == TypeDate || tp == TypeDatetime>::type>
void unflatten(const Field & orig, std::optional<Field> & copy)
{
    if (orig.isNull())
        return;
...
template <TP tp, typename std::enable_if<tp == TypeDate || tp == TypeDatetime>::type>
void flatten(const Field & orig, std::optional<Field> & copy)
{
    if (orig.isNull())
        return;
...
template <typename T>
inline bool integerOverflow(const Field & field, const ColumnInfo & column_info)
{
...
template <TP tp, typename std::enable_if<tp == TypeTiny || tp == TypeShort || tp == TypeLong || tp == TypeInt24>::type>
bool overflow(const Field & field, const ColumnInfo & column_info)
{
    if (field.isNull())
        return false;

    if constexpr (tp == TypeTiny)
        return integerOverflow<Int8>(field, column_info);

    if constexpr (tp == TypeShort)
        return integerOverflow<Int16>(field, column_info);

    if constexpr (tp == TypeLong || tp == TypeInt24)
        return integerOverflow<Int32>(field, column_info);
}
...
bool DatumFlat::isOverflow(const ColumnInfo & column_info)
{
    switch (tp)
    {
#ifdef M
...
#define M(tt, v, cf, ct, w) \
    case Type##tt:          \
        return overflow<Type##tt>(field(), column_info);
        COLUMN_TYPES(M)
#undef M
    }
