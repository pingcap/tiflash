...
{
    return context.getTiFlashContext().getIgnoreDatabases().count(database_name) > 0;
}
...
void SchemaBuilder<Getter, NameMapper>::applyAlterTable(DBInfoPtr db_info, TableID table_id)
{
    ...
    if (storage == nullptr)
    {
        throw TiFlashException(
            "miss table in TiFlash : " + name_mapper.debugCanonicalName(*db_info, *table_info),
...
void SchemaBuilder<Getter, NameMapper>::applyDropSchema(DatabaseID schema_id)
{
    ...
    if (unlikely(it == databases.end()))
    {
        LOG_INFO(
            log,
            "Syncer wants to drop database [id=" << std::to_string(schema_id) << "], but database is not found, may has been dropped.");
        return;
    }
...
void SchemaBuilder<Getter, NameMapper>::applySetTiFlashReplica(TiDB::DBInfoPtr db_info, TableID table_id)
{
    ...
    {
        throw TiFlashException(
            "miss table in TiFlash : " + name_mapper.debugCanonicalName(*db_info, *latest_table_info),
            Errors::DDL::MissingTable);
    }
