...
RandomAccessFilePtr EncryptedFileProvider::newRandomAccessFileImpl(const std::string & file_name_, int flags)
{
    ...
    return std::make_shared<EncryptedRandomAccessFile>(underlying, encryption_provider->createCipherStream(file_name_));
}
...
WritableFilePtr EncryptedFileProvider::newWritableFileImpl(const std::string & file_name_, int flags, mode_t mode)
{
    ...
    return std::make_shared<EncryptedWritableFile>(underlying, encryption_provider->createCipherStream(file_name_));
}
} // namespace DB
