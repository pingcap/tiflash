                                  ...
                                  int flags) const
{
    ...
    if (file_info.method != EncryptionMethod::Plaintext)
    {
        file = std::make_shared<EncryptedRandomAccessFile>(file, createCipherStream(file_info, encryption_path_));
    }
    return file;
}
                                              ...
                                              mode_t mode) const
{
    ...
    if (encryption_enabled)
    {
        if (create_new_encryption_info_)
        {
            ...
            file = std::make_shared<EncryptedWritableFile>(file, createCipherStream(file_info, encryption_path_));
        }
        else
        {
            auto file_info = key_manager->getFile(encryption_path_.dir_name);
