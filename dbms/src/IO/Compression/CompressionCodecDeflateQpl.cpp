    ...
    , distribution(0, MAX_HW_JOB_NUMBER - 1)
{
    ...
    {
        auto * qpl_job_ptr = reinterpret_cast<qpl_job *>(hw_jobs_buffer.get() + index * job_size);
        if (auto status = qpl_init_job(qpl_path_hardware, qpl_job_ptr); status != QPL_STS_OK)
        {
            job_pool_ready = false;
            LOG_WARNING(
                log,
                "Initialization of hardware-assisted DeflateQpl codec failed: {} , falling back to software DeflateQpl "
                "codec. Please check if Intel In-Memory Analytics Accelerator (IAA) is properly set up. QPL Version: "
                "{}.",
...
HardwareCodecDeflateQpl::HardwareCodecDeflateQpl()
    : log(Logger::get("HardwareCodecDeflateQpl"))
{}
...
Int32 HardwareCodecDeflateQpl::doCompressData(const char * source, UInt32 source_size, char * dest, UInt32 dest_size)
    const
{
    UInt32 job_id = 0;
    ...
    if (!(job_ptr = DeflateQplJobHWPool::instance().acquireJob(job_id)))
    {
        LOG_INFO(
            log,
            "DeflateQpl HW codec failed, falling back to SW codec.(Details: doCompressData->acquireJob fail, probably "
            "job pool exhausted)");
        return RET_ERROR;
    }
    ...
    else
    {
        LOG_WARNING(
            log,
            "DeflateQpl HW codec failed, falling back to SW codec.(Details: doCompressData->qpl_execute_job with error "
            "code: {} - please refer to qpl_status in ./contrib/qpl/include/qpl/c_api/status.h)",
            static_cast<UInt32>(status));
        DeflateQplJobHWPool::instance().releaseJob(job_id);
        return RET_ERROR;
    }
...
Int32 HardwareCodecDeflateQpl::doDecompressDataSynchronous(
    const char * source,
    UInt32 source_size,
    char * dest,
    UInt32 uncompressed_size)
{
    ...
    if (!(job_ptr = DeflateQplJobHWPool::instance().acquireJob(job_id)))
    {
        LOG_INFO(
            log,
            "DeflateQpl HW codec failed, falling back to SW codec.(Details: doDecompressDataSynchronous->acquireJob "
            "fail, probably job pool exhausted)");
        return RET_ERROR;
    }
    ...
    if (auto status = qpl_submit_job(job_ptr); status != QPL_STS_OK)
    {
        ...
        LOG_WARNING(
            ...
            "doDecompressDataSynchronous->qpl_execute_job with error code: {} - please refer to qpl_status in "
...
void SoftwareCodecDeflateQpl::doDecompressData(
    const char * source,
    UInt32 source_size,
    char * dest,
    UInt32 uncompressed_size)
...
UInt32 CompressionCodecDeflateQpl::getMaxCompressedDataSize(UInt32 uncompressed_size) const
{
    /// Aligned with ZLIB
    return (
        (uncompressed_size) + ((uncompressed_size) >> 12) + ((uncompressed_size) >> 14) + ((uncompressed_size) >> 25)
        + 13);
}
...
void CompressionCodecDeflateQpl::doDecompressData(
    const char * source,
    UInt32 source_size,
    char * dest,
