...
template <typename T>
size_t CompressionCodecBitPacking::compressDataForType(const char * source, UInt32 source_size, char * dest) const
{
...
template <typename T>
void CompressionCodecBitPacking::decompressDataForType(
    const char * source,
    UInt32 source_size,
    char * dest,
...
template <typename T>
CompressionCodecBitPacking::Mode CompressionCodecBitPacking::analyze(std::vector<T> & values, State<T> & state) const
{
    ...
    size_t copy_size = values.size() * sizeof(T);
    size_t rle_size = rle.size() * (sizeof(T) + sizeof(UInt16));
    if (rle_size < delta_for_size && rle_size < for_size && rle_size < copy_size)
    {
        state = std::move(rle);
        return Mode::RLE;
    }
    else if (for_size < delta_for_size && for_size < copy_size)
    {
        state = FORState<T>{min_value, for_width};
        return Mode::FOR;
    }
    else if (delta_for_size < copy_size)
    {
