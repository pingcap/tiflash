...
WriteBufferFromFileBase * createWriteBufferFromFileBase(FileProviderPtr & file_provider, const std::string & filename_,
                const EncryptionPath & encryption_path_, size_t estimated_size, size_t aio_threshold, size_t buffer_size_, int flags_, mode_t mode, char * existing_memory_,
        size_t alignment)
{
    if ((aio_threshold == 0) || (estimated_size < aio_threshold))
    {
        ProfileEvents::increment(ProfileEvents::CreatedWriteBufferOrdinary);
        return new WriteBufferFromFileProvider(file_provider, filename_, encryption_path_, buffer_size_, flags_, mode, existing_memory_, alignment);
    }
    else
    {
