...
AggregateFunctionPtr createAggregateFunctionAvgForDecimal(const IDataType * p) 
{
    if (auto dec_type = typeid_cast<const T * >(p)) {
        ...
        if (checkDecimal<Decimal32>(*result_type))
            res = AggregateFunctionPtr(createWithDecimalType<AggregateFunctionAvg, Decimal32>(*dec_type, dec_type->getPrec(), dec_type->getScale(), result_prec, result_scale));
        else if (checkDecimal<Decimal64>(*result_type))
            res = AggregateFunctionPtr(createWithDecimalType<AggregateFunctionAvg, Decimal64>(*dec_type, dec_type->getPrec(), dec_type->getScale(), result_prec, result_scale));
        else if (checkDecimal<Decimal128>(*result_type))
            res = AggregateFunctionPtr(createWithDecimalType<AggregateFunctionAvg, Decimal128>(*dec_type, dec_type->getPrec(), dec_type->getScale(), result_prec, result_scale));
        else
            res = AggregateFunctionPtr(createWithDecimalType<AggregateFunctionAvg, Decimal256>(*dec_type, dec_type->getPrec(), dec_type->getScale(), result_prec, result_scale));
        return res;
