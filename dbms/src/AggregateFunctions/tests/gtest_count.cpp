...
ColumnWithTypeAndName real_col_all_null_value = createColumn<Nullable<double>>({{}, {}, {}, {}});
...
try
{
    ...
    ASSERT_COLUMN_EQ(
        createColumn<UInt64>({4}),
        executeAggregateFunction("count",
                                 ColumnsWithTypeAndName{real_col_all_null_value},
                                 ColumnNumbers{}));

    // select count(col) where id >= 7 group by group_col_1
    ASSERT_COLUMN_EQ(
        createColumn<UInt64>({4}),
        executeAggregateFunction("count",
    ...
    ASSERT_COLUMN_EQ(
        createColumn<UInt64>({4}),
        executeAggregateFunction("count",
                                 ColumnsWithTypeAndName{real_col_all_null_value, group_col_all_null_value},
                                 ColumnNumbers{1}));
