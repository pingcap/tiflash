...
Int64 IProfilingBlockInputStream::getWorkingTime()
{
    ...
    if (!children.empty())
    {
        auto child_it = children.cbegin();
        if (IProfilingBlockInputStream * p_first = dynamic_cast<IProfilingBlockInputStream *>(child_it->get()))
        {
            working_time += p_first->getWorkingTime();
            if (dynamic_cast<SharedQueryBlockInputStream *>(p_first))
            {
                return working_time;
            }

            ++child_it;
            while (child_it != children.cend())
            {
                if (IProfilingBlockInputStream * p_child = dynamic_cast<IProfilingBlockInputStream *>(child_it->get()))
                {
                    working_time += p_child->getWorkingTime();
                }
                ++child_it;
            }
        }
