...
struct IsTriviallyMoveConstructibleObject
    : std::integral_constant<
          bool,
          std::is_move_constructible<type_traits_internal::SingleMemberUnion<T>>::value
...
struct IsTriviallyCopyConstructibleObject
    : std::integral_constant<
          bool,
          std::is_copy_constructible<type_traits_internal::SingleMemberUnion<T>>::value
              && phmap::is_trivially_destructible<T>::value>
{
};
...
struct is_function
    : std::integral_constant<
          bool,
          !(std::is_reference<T>::value || std::is_const<typename std::add_const<T>::type>::value)>
{
};
...
struct common_params
{
    ...
    using node_count_type = phmap::conditional_t<
        (kNodeValueSpace / sizeof(value_type) > (std::numeric_limits<uint8_t>::max)()),
        uint16_t,
        uint8_t>; // NOLINT
...
public:
    ...
    {
        static_assert(
            LeafLayout(1).Alignment() == InternalLayout().Alignment(),
...
public:
    ...
    SearchResult<int, btree_is_key_compare_to<Compare, key_type>::value> linear_search(
        const K & k,
    ...
    template <typename K, typename Compare>
    SearchResult<int, btree_is_key_compare_to<Compare, key_type>::value> binary_search(
        const K & k,
        const Compare & comp) const
    {
...
void btree_node<P>::rebalance_left_to_right(const int to_move, btree_node * right, allocator_type * alloc)
{
    ...
    else
    {
        ...
        params_type::move(
            alloc,
            slot(count() - (to_move - 1)),
            slot(count() - uninitialized_remaining),
            right->slot(0));
    }
...
{
    static_assert(
        std::is_same<btree, Btree>::value || std::is_same<const btree, Btree>::value,
...
public:
    ...
    iterator insert(const_iterator hint, node_type && node)
    {
        ...
        std::pair<iterator, bool> res = this->tree_.insert_hint_unique(
            iterator(hint),
            params_type::key(CommonAccess::GetSlot(node)),
