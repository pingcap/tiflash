...
[[maybe_unused]] bool searchAndCollectSymbolsFromELFSymbolTable(
    dl_phdr_info * info,
    const Elf & elf,
...
void collectSymbolsFromELF(
    dl_phdr_info * info,
    [[maybe_unused]] std::vector<SymbolIndex::Symbol> & symbols,
    [[maybe_unused]] std::vector<SymbolIndex::Object> & objects,
    String & build_id)
{
    ...
    Poco::Path canonical_path = Poco::Path(object_name).absolute();
    if (!Poco::File(canonical_path).exists())
        return;

#if 0 // TODO: collect symbol from debug binary
