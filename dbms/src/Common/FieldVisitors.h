...
static typename std::decay_t<Visitor>::ResultType applyBinaryVisitorImpl(Visitor && visitor, F1 && field1, F2 && field2)
{
    switch (field2.getType())
    {
        ...
        case Field::Types::Decimal256:   return visitor(field1, field2.template get<DecimalField<Decimal256>>());
...
typename std::decay_t<Visitor>::ResultType applyVisitor(Visitor && visitor, F1 && field1, F2 && field2)
{
    switch (field1.getType())
    {
        ...
        case Field::Types::Decimal256:
            return applyBinaryVisitorImpl(
                std::forward<Visitor>(visitor), field1.template get<DecimalField<Decimal256>>(), std::forward<F2>(field2));
...
public:
    ...
    template<typename U>
    T operator() (const DecimalField<U> & x) const { return static_cast<T>(x.getValue().value); }
};
...
public:
    ...
    template <typename T> bool operator() (const UInt64 & l, const DecimalField<T> & r) const { return DecimalField<Decimal256>(static_cast<Int256>(l), 0) == r; }
    template <typename T> bool operator() (const Int64 & l, const DecimalField<T> & r) const { return DecimalField<Decimal256>(static_cast<Int256>(l), 0) == r; }
