...
#define CHECK_SUMBIT                                                                          \
    do                                                                                        \
    {                                                                                         \
        if (unlikely(MEMORY_TRACER_LOCAL_DELTA.delta > MEMORY_TRACER_SUBMIT_THRESHOLD))       \
        {                                                                                     \
            if (current_memory_tracker)                                                       \
                current_memory_tracker->alloc(MEMORY_TRACER_LOCAL_DELTA.delta);               \
            MEMORY_TRACER_LOCAL_DELTA.delta = 0;                                              \
        }                                                                                     \
        else if (unlikely(MEMORY_TRACER_LOCAL_DELTA.delta < -MEMORY_TRACER_SUBMIT_THRESHOLD)) \
        {                                                                                     \
            if (current_memory_tracker)                                                       \
