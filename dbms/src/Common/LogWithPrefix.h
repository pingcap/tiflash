...
{
public:
    static LogWithPrefixPtr get(const std::string & source)
    {
        return std::make_shared<LogWithPrefix>(source, "");
    }

    static LogWithPrefixPtr get(const std::string & source, const std::string & identifier)
    {
        return std::make_shared<LogWithPrefix>(source, identifier);
    }

    template <typename... Args>
    static LogWithPrefixPtr get(const std::string & source, Args &&... args)
    {
        FmtBuffer buf;
...
private:
    ...
    static LogWithPrefixPtr getInternal(const std::string & source, FmtBuffer & buf, T && first, Args &&... args)
    {
    ...
    template <typename T>
    static LogWithPrefixPtr getInternal(const std::string & source, FmtBuffer & buf, T && identifier)
    {
