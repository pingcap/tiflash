...
struct MatchResult {
    ...
    std::string toString() const {
        ...
        for (const auto & [region_id, region]: regions) {
            fmt_buf.fmtAppend("region {} {}, tikv_range: {}", region_id, region->getDebugString(), region->getRange()->toDebugString());
        }
...
BlockInputStreamPtr dbgFuncFindKeyDt(Context & context, const ASTs & args) {
    ...
    auto key = typeid_cast<const ASTLiteral &>(*args[2]).value;
    auto value = typeid_cast<const ASTLiteral &>(*args[3]).value;
    if(table_info.pk_is_handle) {
    } else {
    }
    auto query = fmt::format("selraw *,_INTERNAL_VERSION from {}.{} where {} = {}", database_name, table_name, key, value);
