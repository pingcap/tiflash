...
struct Join : Executor
{
    ...
    Join(size_t & index_, const DAGSchema & output_schema_, tipb::JoinType tp_, const ASTs & l_conds, const ASTs & r_conds, const ASTs & o_conds, const ASTs & o_eq_conds)
        : Executor(index_, "Join_" + std::to_string(index_), output_schema_)
        , tp(tp_)
        , left_conds(l_conds)
        , right_conds(r_conds)
    ...
    {
        if (!(left_conds.size() + right_conds.size() + other_conds.size() + other_eq_conds_from_in.size()))
            throw Exception("No join condition found.");
    }
...
ExecutorPtr compileJoin(size_t & executor_index, ExecutorPtr left, ExecutorPtr right, tipb::JoinType tp, const ASTs & left_conds, const ASTs & right_conds, const ASTs & other_conds, const ASTs & other_eq_conds_from_in);
