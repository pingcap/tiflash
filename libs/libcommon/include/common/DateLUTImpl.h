...
private:
    /// Lookup table is indexed by DayNum_t.
    ...
    inline DayNum_t findIndex(time_t t) const
    {
        /// First guess.
        DayNum_t guess(t / 86400);

        /// UTC offset is from -12 to +14 in all known time zones. This requires checking only three indices.

        if ((guess == 0 || t >= lut[guess].date) && t < lut[DayNum_t(guess + 1)].date)
            return guess;
...
public:
    ...
    inline time_t toFirstDayOfWeek(time_t t) const
    {
        ...
        return lut[DayNum_t(index - (lut[index].day_of_week - 1))].date;
    }
    ...
    inline UInt8 daysInMonth(UInt16 year, UInt8 month) const
    {
        ...
        DayNum_t any_day_of_month = DayNum_t(years_lut[year - DATE_LUT_MIN_YEAR] + 32 * (month - 1));
    ...
    {
        return lut[DayNum_t(findIndex(t) + days)].date;
    }
    ...
    inline DayNum_t toDayNum(time_t t) const { return findIndex(t); }
    ...
    inline unsigned toDayOfYear(DayNum_t d) const { return d + 1 - toFirstDayNumOfYear(d); }
    ...
    /// (round down to monday and divide DayNum_t by 7; we made an assumption,
    ...
    inline unsigned toISOYear(DayNum_t d) const
    {
        /// That's effectively the year of thursday of current week.
        return toYear(DayNum_t(d + 4 - toDayOfWeek(d)));
    }
    ...
    inline DayNum_t toFirstDayNumOfISOYear(DayNum_t d) const
    {
        auto iso_year = toISOYear(d);

        DayNum_t first_day_of_year = years_lut[iso_year - DATE_LUT_MIN_YEAR];
        auto first_day_of_week_of_year = lut[first_day_of_year].day_of_week;

        return DayNum_t(first_day_of_week_of_year <= 4
            ? first_day_of_year + 1 - first_day_of_week_of_year
            : first_day_of_year + 8 - first_day_of_week_of_year);
    }

    inline DayNum_t toFirstDayNumOfISOYear(time_t t) const
    {
    ...
    inline unsigned toISOWeek(DayNum_t d) const
    {
        return 1 + DayNum_t(toFirstDayNumOfWeek(d) - toFirstDayNumOfISOYear(d)) / 7;
    }
    ...
    /*
      ...
      WeekModeFlag::NEWYEAR_DAY (3)   If not set  no meaning
        ...
        next week is week 1.
    */
    inline YearWeek toYearWeek(DayNum_t d, UInt8 week_mode) const
    {
        ...
        UInt16 weekday = calc_weekday(DayNum_t(first_daynr), !monday_first_mode);
    ...
    inline YearWeek toYearWeekOfNewyearMode(DayNum_t d, bool monday_first_mode) const
    {
        YearWeek yw(0, 0);
        UInt16 offset_day = monday_first_mode ? 0U : 1U;

        // Checking the week across the year
        yw.first = toYear(DayNum_t(d + 7 - toDayOfWeek(DayNum_t(d + offset_day))));

        DayNum_t first_day = makeDayNum(yw.first, 1, 1);
        DayNum_t this_day = d;
        ...
        else
        {
            ...
            if (toDayOfWeek(first_day) != 7)
                first_day = DayNum_t(first_day - toDayOfWeek(first_day));
            if (toDayOfWeek(d) != 7)
                this_day = DayNum_t(d - toDayOfWeek(d));
        }
     ...
     */
    inline DayNum_t toFirstDayNumOfWeek(DayNum_t d, UInt8 week_mode) const
    {
        bool monday_first_mode = week_mode & static_cast<UInt8>(WeekModeFlag::MONDAY_FIRST);
        if (monday_first_mode)
        {
        ...
        {
            return (toDayOfWeek(d) != 7) ? DayNum_t(d - toDayOfWeek(d)) : d;
        }
    }
     ...
     */
    inline unsigned calc_weekday(DayNum_t d, bool sunday_first_day_of_week) const
    {
        if (!sunday_first_day_of_week)
            return toDayOfWeek(d) - 1;
        else
            return toDayOfWeek(DayNum_t(d + 1)) - 1;
    }
    ...
    inline DayNum_t toStartOfYearInterval(DayNum_t d, UInt64 years) const
    {
        if (years == 1)
    ...
    inline DayNum_t toStartOfQuarterInterval(DayNum_t d, UInt64 quarters) const
    {
        if (quarters == 1)
    ...
    inline DayNum_t toStartOfMonthInterval(DayNum_t d, UInt64 months) const
    {
        if (months == 1)
    ...
    inline DayNum_t toStartOfWeekInterval(DayNum_t d, UInt64 weeks) const
    {
        if (weeks == 1)
            return toFirstDayNumOfWeek(d);
        UInt64 days = weeks * 7;
        // January 1st 1970 was Thursday so we need this 4-days offset to make weeks start on Monday.
        return DayNum_t(4 + (d - 4) / days * days);
    }

    inline time_t toStartOfDayInterval(DayNum_t d, UInt64 days) const
    {
        if (days == 1)
            return toDate(d);
    ...
    inline DayNum_t addQuarters(DayNum_t d, Int64 delta) const
    {
